---
name: retrospect
description: 'Structured retrospective on a shipped commit range, release, PR, commit, or session: per-fix validation, evidence-sourcing pass, re-diagnosis, action verdicts, growing failure-mode library. Triggers — "/aria-cowork:retrospect", "retrospect this". (Cowork variant — namespaced-only.)'
...
Run a structured retrospective on a shipped commit range (or single commit, release, deployment, PR, or current session). Produces a 10-section markdown report with per-fix verdicts, validation status, action recommendations, and re-diagnosis when fixes failed. Writes findings to `<knowledge_folder>/logs/retrospect/` and runs aria's standard intake.

**Cowork variant of aria-knowledge's `/retrospect`.** Output schema (10-section report shape, frontmatter, logs/retrospect/ destination) byte-identical per #19i + #19j. Three input-discovery divergences (per #19c + #19e + #19f):

- **Git-bound scopes use user-paste fallback instead of shelling git** (#19c). aria-knowledge runs `git log`, `git diff`, `gh pr view`, etc. directly for `commit` / `range` / `pr` / `deployment` scopes. Cowork has no shell access — these scopes prompt the user to paste git output at scope-start (e.g., *"Paste `git log --oneline <range>` output"*, *"Paste `gh pr view <num>` output"*). Once data is in conversation context, the downstream analysis is identical to aria-knowledge. **Session and decision scopes work unmodified** — they don't require git.
- **Step 0.5 ledger uses in-memory dedup if `/tmp/` is not writable** (#19e + cross-ref to #18d). Same shape as `/aria-cowork:prospect` Step 0.5 substep 7 — opportunistic `/tmp/aria-active-*` check + in-skill in-memory fallback.
- **Step 11 Tracked artifacts surfacing (CODEMAP/STITCH) SKIPPED per ADR-005 + #19f.** Same shape as `/aria-cowork:prospect` Step 11 skip — cowork excludes `/codemap` + `/stitch`, retrospective verdict doesn't depend on codebase-structure validation.
...
**Before Step 0 — runtime self-correction.** Check whether the `Bash` tool is available in this session. If `Bash` IS available (you are running in Claude Code or another runtime with shell access), this invocation reached the wrong variant for the runtime. **Do not ask — redirect.** Announce it in one line, then use the `Skill` tool to invoke `retrospect` with the same arguments the user provided, and do not run this skill's steps:

> ↪️ **Redirecting to `/retrospect` — you invoked the aria-cowork variant, but you are running in Claude Code or another runtime with shell access.**
>
...
**Cowork scope-handling per #19c (user-paste fallback for git-bound scopes):**

| Scope | Cowork behavior |
|---|---|
| **session** | **Native.** Uses files Claude has touched in the current conversation — no git needed. |
| **decision** | **Native** (cowork-relevant scope). Reviews decisions surfaced in the current conversation — no git needed. |
| **commit** / **range** / **pr** / **release** / **deployment** / **(no args = auto-range)** | **User-paste fallback.** Cowork has no shell access to run `git log`, `git diff`, `gh pr view`, `gh release view`, etc. At scope-start, prompt the user to paste the equivalent output. Example prompts: *"Paste the output of `git log --oneline <range>` (or just the commit subjects, one per line):"*, *"Paste `gh pr view <num>` JSON or human-readable output:"*. Accept whatever shape the user provides and parse the commit SHAs / subjects / file lists from there. Once data is in conversation context, downstream Step 1+ analysis is IDENTICAL to aria-knowledge — same anchor block, same pattern-library lookup, same Evidence-Sourcing Pass, same verdicts. |

If a git command appears in the scope-table or body below, treat it as **aria-knowledge documentation** — cowork prompts the user to paste the equivalent output instead of running the command. The skill's analytical logic doesn't care where the git data came from (shell or paste); only the acquisition path differs.
...
7. **Ledger filter (best-effort, cowork-fallback to in-memory per #19e).** Resolve the ledger at `/tmp/aria-active-<session-id>`, where `<session-id>` is your runtime's session identifier (in Claude Code, `$CLAUDE_CODE_SESSION_ID`). If it exists and is readable, drop already-listed paths. ⛔ Never use `ls -t` over the ledger glob to guess — it returns the most recently modified ledger on the MACHINE, i.e. another live session's whenever yours is absent, and a foreign ledger DROPS files never surfaced to this session. If no session identifier is available OR the Bash command fails OR no ledger exists OR Cowork's sandbox restricts `/tmp/` writes, fall back to **in-skill in-memory dedup**: maintain a list of files already loaded earlier in THIS skill's invocation (across Step 0.5's 10 substeps) and skip re-reads. Per-invocation scope, not per-session — cross-skill dedup is best-effort in cowork (no hook layer pre-populates the ledger).
...
11. **Tracked artifacts surfacing — SKIPPED in cowork per #19f + ADR-005.** aria-knowledge surfaces CODEMAP directory + STITCH file for the analyzed range's project as Step 11 (detects project tag from `git diff --name-only`; CODEMAP staleness-checked against `codemap_staleness_threshold_days`; STITCH against `stitch_staleness_threshold_days`; ledger-deduped via `/tmp/aria-active-*`). Cowork excludes `/codemap` + `/stitch` skills per ADR-005 AND cowork can't run `git diff` directly — so this step is dropped entirely. The Step 9 surfacing block does NOT add the "Tracked artifacts: ..." 4th line in cowork. Retrospective verdict in cowork doesn't depend on codebase-structure validation — if a retro needs CODEMAP-grounded analysis, run aria-knowledge's `/retrospect` from Claude Code.
...
Read the canonical pattern library at `<knowledge_folder>/rules/retrospect-patterns.md` (resolved per `<knowledge_folder>/aria-config.md` `knowledge_folder`).
...
### Default (configurable in `<knowledge_folder>/aria-config.md` under `retrospect:` block — to be added when needed)
