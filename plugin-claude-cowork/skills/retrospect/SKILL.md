...
7. **Ledger filter (best-effort, cowork-fallback to in-memory per #19e).** Try `ls -t /tmp/aria-active-* 2>/dev/null | head -1` via Bash to find the current session's ledger. If found and readable, drop already-listed paths. If the Bash command fails OR no ledger exists OR Cowork's sandbox restricts `/tmp/` writes, fall back to **in-skill in-memory dedup**: maintain a list of files already loaded earlier in THIS skill's invocation (across Step 0.5's 10 substeps) and skip re-reads. Per-invocation scope, not per-session — cross-skill dedup is best-effort in cowork (no hook layer pre-populates the ledger).
