...
Run a structured pre-mortem on a plan or approach that has been *created but not yet executed*. Forward-looking counterpart to `/aria-cowork:retrospect`. Produces a 10-section markdown report with per-step verdicts, risk status, action recommendations, and process pre-mortem when the plan-formation itself was thin. Writes findings to `<knowledge_folder>/logs/prospect/` and runs aria's standard intake.

**Cowork variant of aria-knowledge's `/prospect`.** Output schema (10-section report shape, frontmatter, logs/prospect/ destination) byte-identical per #18a + #18g + #18h. Two input-discovery divergences:

- **Step 11 Tracked artifacts surfacing (CODEMAP/STITCH) SKIPPED per ADR-005 + #18f.** aria-knowledge surfaces CODEMAP directory + STITCH file for the plan's project as Step 11. Cowork excludes `/codemap` + `/stitch` skills, so this step is dropped entirely. Plan-quality verdict in cowork doesn't depend on codebase-structure validation.
- **Step 0.5 ledger uses in-memory dedup if `/tmp/` is not writable** (#18d). aria-knowledge's Step 0.5 Active Knowledge Surfacing dedups against `/tmp/aria-active-{session_id}` to avoid re-Reading files already surfaced by hooks in the same session. Cowork has no hook layer (no upstream surfacing to dedup against), and Cowork's sandbox may restrict `/tmp/` writes — cowork falls back to in-skill in-memory dedup when ledger file ops fail.
...
**Canonical resolution:** This is the Claude Cowork variant — namespaced-only. When both `plugin-claude-code` and `plugin-claude-cowork` are loaded in the same session (most common in Claude Desktop), bare `/prospect` resolves to aria-knowledge's variant — Code is the canonical owner of all 24 dual-port skills per ADR-094 §Part 1. To reach this skill, use the namespaced form: `/aria-cowork:prospect`. Do NOT match bare `/prospect` — that belongs to aria-knowledge.

**Before Step 0 — runtime self-correction.** Check whether the `Bash` tool is available in this session. If `Bash` IS available (you are running in Claude Code or another runtime with shell access), this invocation reached the wrong variant for the runtime. **Do not ask — redirect.** Announce it in one line, then use the `Skill` tool to invoke `prospect` with the same arguments the user provided, and do not run this skill's steps:

> ↪️ **Redirecting to `/prospect` — you invoked the aria-cowork variant, but you are running in Claude Code or another runtime with shell access.**
>
...
7. **Ledger filter (best-effort, cowork-fallback to in-memory).** aria-knowledge resolves the ledger at `/tmp/aria-active-<session-id>`, where `<session-id>` is the runtime's session identifier (in Claude Code, `$CLAUDE_CODE_SESSION_ID`) — written by aria-knowledge's hooks (T-1/T-2/T-3/PostCompact) to track files already surfaced this session. Cowork has no hook layer that pre-populates this ledger AND Cowork's sandbox may restrict `/tmp/` writes. Try the same resolution opportunistically; if the file exists and reads cleanly, drop already-listed paths. ⛔ Never use `ls -t` over the ledger glob to guess — it returns the most recently modified ledger on the MACHINE, i.e. another live session's whenever yours is absent, and a foreign ledger DROPS files never surfaced to this session. If no session identifier is available OR the Bash command fails OR no ledger exists, fall back to **in-skill in-memory dedup**: maintain a list of files already loaded earlier in THIS skill's invocation (i.e., across the 10 substeps of Step 0.5), and skip re-reading any path that appears twice. The fallback is per-invocation, not per-session — cross-skill dedup is best-effort only in cowork.
...
11. **Tracked artifacts surfacing — SKIPPED in cowork per #18f + ADR-005.** aria-knowledge surfaces CODEMAP directory + STITCH file for the plan's project as Step 11 (CODEMAP staleness-checked against `codemap_staleness_threshold_days`; STITCH against `stitch_staleness_threshold_days`; ledger-deduped via `/tmp/aria-active-*`). Cowork excludes `/codemap` + `/stitch` skills per ADR-005, so this step is dropped entirely. The Step 9 surfacing block does NOT add the "Tracked artifacts: ..." 4th line in cowork. Plan-quality verdict in cowork doesn't depend on codebase-structure validation — if a plan needs CODEMAP-grounded analysis, run aria-knowledge's `/prospect` from Claude Code.
