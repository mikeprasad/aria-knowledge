...
Read the canonical pattern library at `~/knowledge/rules/retrospect-patterns.md` (resolved per `<knowledge_folder>/aria-config.md` `knowledge_root`). The retrospect pattern library is intentionally shared — most failure-mode patterns (theory-driven refactor, scope creep, abstraction-first, etc.) apply forward as well as backward.
