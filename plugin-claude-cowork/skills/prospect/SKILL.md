...
7. **Ledger filter (best-effort, cowork-fallback to in-memory).** aria-knowledge runs `ls -t /tmp/aria-active-* 2>/dev/null | head -1` to find the current session's ledger (the most recently modified file matching that pattern) — written by aria-knowledge's hooks (T-1/T-2/T-3/PostCompact) to track files already surfaced this session. Cowork has no hook layer that pre-populates this ledger AND Cowork's sandbox may restrict `/tmp/` writes. Try the same Bash command opportunistically; if it returns a ledger and the ledger reads cleanly, drop already-listed paths. If the Bash command fails OR no ledger exists, fall back to **in-skill in-memory dedup**: maintain a list of files already loaded earlier in THIS skill's invocation (i.e., across the 10 substeps of Step 0.5), and skip re-reading any path that appears twice. The fallback is per-invocation, not per-session — cross-skill dedup is best-effort only in cowork.
