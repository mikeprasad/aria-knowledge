...
Scan the attached knowledge folder for extractable items, route ideas via the Accept submenu, archive cleared content per the universal audit-cohort conventions, and rebuild the index.

**Cowork variant of aria-knowledge's `/audit-knowledge`.** Output schema is byte-identical (`archive/audit-{date}/MANIFEST.md`, disposition frontmatter, ledger formats) per item #4 archive-cohort conventions. Three input-discovery divergences:

- **Memory + plans scan SKIPPED in cowork** (#16b, #16c locks): aria-knowledge reads `~/.claude/projects/{cwd}/memory/` and `~/.claude/plans/` to surface extractable items from auto-memory + Plan files. Cowork's persistent-grant model can't reach `~/.claude/` per probe 12. Cowork's `/audit-knowledge` audits the attached knowledge folder's `intake/` subfolder only. If you want memory/plans audited, run aria-knowledge's `/audit-knowledge` from Claude Code.
- **`bin/` shell-outs replaced with Claude-driven inline logic** (per D3 + ADR-013): aria-knowledge sources `bin/config.sh` for shell helpers and uses `bin/digest-transcript.sh` for snapshot processing. Cowork has no `bin/` directory; equivalent operations are done inline by Claude reading config + processing snapshot bodies directly.
- **`projects_groups` features not consumed in cowork** (per ADR-005): aria-knowledge integrates `/distill`, `/stitch`, and codemap surfaces into the audit. Cowork excludes those skills; related audit branches are noted as aria-knowledge-only.
...
**Canonical resolution:** This is the Claude Cowork variant — namespaced-only. When both `plugin-claude-code` and `plugin-claude-cowork` are loaded in the same session (most common in Claude Desktop), bare `/audit-knowledge` resolves to aria-knowledge's variant — Code is the canonical owner of all 24 dual-port skills per ADR-094 §Part 1. To reach this skill, use the namespaced form: `/aria-cowork:audit-knowledge`. Do NOT match bare `/audit-knowledge` — that belongs to aria-knowledge.

**Before Step 0 — runtime self-correction.** Check whether the `Bash` tool is available in this session. If `Bash` IS available (you are running in Claude Code or another runtime with shell access), this invocation reached the wrong variant for the runtime. **Do not ask — redirect.** Announce it in one line, then use the `Skill` tool to invoke `audit-knowledge` with the same arguments the user provided, and do not run this skill's steps:

> ↪️ **Redirecting to `/audit-knowledge` — you invoked the aria-cowork variant, but you are running in Claude Code or another runtime with shell access.**
>
...
Read `<knowledge_folder>/aria-config.md` and extract:
...
If the config file doesn't exist, stop: "aria-knowledge is not configured. Run /aria-setup to get started."
...
Read `{knowledge_folder}/intake/insights-backlog.md`. **If the file is missing**, report it in Step 6 and suggest running `/aria-setup` to repair the structure. Do not create it.
...
Read `{knowledge_folder}/intake/decisions-backlog.md`. **If the file is missing**, report it in Step 6 and suggest running `/aria-setup` to repair the structure. Do not create it.
...
Read `{knowledge_folder}/intake/extraction-backlog.md`. **If the file is missing**, report it in Step 6 and suggest running `/aria-setup` to repair the structure. Do not create it.
...
Glob `{knowledge_folder}/intake/ideas/*.md`. **If the directory is missing**, report it in Step 6 and suggest running `/aria-setup` to repair the structure. Do not create it.

**Legacy-file detection (one-time):** Also check for `{knowledge_folder}/intake/ideas-backlog.md`. If it exists alongside `intake/ideas/`, surface a finding in Step 6 under a "Legacy Ideas Backlog" note: *"Pre-2.11 `ideas-backlog.md` detected. Re-run `/aria-setup` to migrate entries into per-file format."* Cowork has no `bin/migrate-ideas-backlog.sh` shell helper (aria-knowledge-only); cowork's `/aria-setup` handles the migration inline when it detects the legacy file. Do not attempt the migration from within the audit flow.
...
**Age annotation and stale marker:** For each idea file, compute age as `(today - idea date)` in days. Derive the idea date as follows: (1) read `date:` from YAML frontmatter; (2) if missing or malformed, fall back to the `YYYY-MM-DD` prefix of the filename. Annotate each entry with its age (`filed N days ago`). Read the staleness threshold from `<knowledge_folder>/aria-config.md` (`ideas_staleness_threshold_days`, default 7) via `config.sh` or fallback. When `age > threshold`, append a `[STALE — still relevant?]` marker to the entry and escalate its visual weight in Step 6 (place stale entries first within the Pending Ideas section, and prompt explicitly for Accept/Reject/Defer rather than allowing implicit Defer).
...
Read `{knowledge_folder}/intake/rules-backlog.md`. **If the file is missing**, report it in Step 6 and suggest running `/aria-setup` to repair the structure. Do not create it.
...
**Digest mode (default):** For each transcript snapshot, produce a high-signal digest before scanning. aria-knowledge uses a `bin/digest-transcript.sh` shell helper for this. Cowork has no `bin/` (per ADR-013 D3 — bin/ shell-outs replaced with Claude-driven inline logic), so cowork's `/audit-knowledge` does the digest **inline by Claude**:

1. Read the snapshot file.
2. Extract: all user messages (truncated to ~500 chars each), first/last 5 lines of each assistant turn, all Insight blocks (lines between `★ Insight ──...` markers), and lines containing decision/feedback signal keywords (*decided*, *because*, *don't do*, *yes exactly*, *correct*, *wrong*, *fixed*, *broken*).
3. Drop tool calls and tool results entirely.
4. Compose the digest in-memory; scan from the digest rather than the raw transcript.

Typical reduction: **~97% fewer tokens** vs raw transcript (~1-3K tokens per snapshot vs ~30-50K+ raw). Same shape as aria-knowledge's shell-script output — done in Claude's context instead of via shell.

**Cowork note:** since cowork has no hook layer, pre-compact snapshots in cowork are created manually via `/aria-cowork:snapshot` invocations only — they tend to be far fewer than aria-knowledge's hook-auto-created snapshots. If `intake/pre-compact-captures/` is empty or absent, this whole step is skipped silently.

**Detailed mode:** Read the full transcript snapshots directly without the digest pass. Use this when digest mode missed something in a previous audit or when the session contained complex multi-step work where mid-response context matters. Note: a single snapshot can consume 30-50K+ context tokens, so use sparingly.
...
Cowork's `/audit-knowledge` SKIPS this step. Report it in Step 6 as: *"Memory scan: aria-knowledge-only step (run from Claude Code for memory audit)."*

If you want memory files audited, run aria-knowledge's `/audit-knowledge` from Claude Code. Both plugins write to the same `archive/audit-{date}/` cohort per item #4h, so running both produces a merged-audit history for the day.

For reference, aria-knowledge's A/B/C memory categorization is:
...
Cowork's audit doesn't produce A/B/C memory findings.

## Step 4: Scan Plan Files (aria-knowledge-only — SKIPPED in cowork)

**Cowork divergence per item #16c:** Same shape as Step 3. aria-knowledge reads `~/.claude/plans/` to surface extractable items. Cowork can't reach that path.

Cowork's `/audit-knowledge` SKIPS this step. Report it in Step 6 as: *"Plans scan: aria-knowledge-only step (run from Claude Code for plans audit)."*

For reference, aria-knowledge looks specifically for:
...
Cowork's audit doesn't produce these findings either.
...
- 2026-04-15 (1 day ago) — aria — feature — /aria-setup diff prompts ahead vs diverged
  Cluster: bundle option available — also see "/aria-setup state-aware second run" (2026-04-15) and "/aria-setup test-mode skip re-decided" (2026-04-15) under aria project.
  Proposal: ...
...
**Write the manifest** before executing promotions:

**Cowork-skip (aria-knowledge-only):** the batch-manifest mechanism is part of aria-knowledge's Rule 22 hook-enforcement infrastructure (`bin/config.sh` shell helpers + `pre-edit-check.sh` hook reads the manifest to allow planned writes through). Cowork has no hook layer per ADR-001 and ADR-004, so cowork's `/audit-knowledge` SKIPS this step entirely. Cowork's audit-time writes (promotions, archive cohort files, backlog clears) proceed without the manifest because there's no PreToolUse hook to gate them.

For reference, aria-knowledge writes the manifest via:
...
## Step 8b: Clear Batch Manifest (aria-knowledge-only — SKIPPED in cowork)

After the audit log is written, aria-knowledge clears the batch manifest to unblock default Rule 22 behavior for any edits later in the session.

**Cowork-skip (aria-knowledge-only):** symmetric counterpart to Step 7a's `kt_batch_begin`. Cowork's audit never wrote a manifest in Step 7a (no hook layer), so there's nothing to clear here. Cowork's `/audit-knowledge` SKIPS this step entirely.

For reference, aria-knowledge clears via:
...
In aria-knowledge: safe to call even if Step 7a's `kt_batch_begin` didn't succeed (e.g., jq missing) — the function just removes the manifest file if it exists. If the audit errors out before reaching Step 8b, `session-start-check.sh`'s stale-manifest auto-clear (30-minute threshold) recovers on the next session start so stale manifests don't silently suppress Rule 22 on unrelated edits. Cowork has none of this hook infrastructure so the recovery story doesn't apply.
