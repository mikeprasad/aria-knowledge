...
**Scope:** These are *cross-project* ADRs — decisions that apply across multiple projects. Project-specific ADRs that don't (yet) apply across projects belong in `projects/{project-tag}/decisions/` (opt-in via `/aria-setup`), which use separate per-project sequential numbering (`001-`, `002-`, ...).
