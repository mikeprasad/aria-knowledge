...
if [ -n "$KA_COUNT_MSG" ]; then
  if [ "$KA_DAYS_FIRED" = "true" ]; then
    MESSAGES="${MESSAGES}${KA_COUNT_MSG}(trigger: count=${BACKLOG_COUNT} threshold=${KT_AUDIT_TRIGGER_THRESHOLD}; also ${DAYS_SINCE_KA}d since last audit) Run /audit knowledge? (or /audit for the full menu) "
  else
    MESSAGES="${MESSAGES}${KA_COUNT_MSG}(trigger: count=${BACKLOG_COUNT} threshold=${KT_AUDIT_TRIGGER_THRESHOLD}) Run /audit knowledge? (or /audit for the full menu) "
  fi
elif [ "$KA_DAYS_FIRED" = "true" ]; then
  MESSAGES="${MESSAGES}Knowledge audit due — ${DAYS_SINCE_KA} days since last audit. (trigger: days=${DAYS_SINCE_KA} threshold=${KT_CADENCE_KNOWLEDGE}; backlog=${BACKLOG_COUNT}) Run /audit knowledge? (or /audit for the full menu) "
fi

if [ "$KA_DUE" = "true" ]; then
  MESSAGES="${MESSAGES}No previous Knowledge Audit found. Run /audit knowledge? (or /audit for the full menu) "
fi
...
MESSAGES="${MESSAGES}RULE 22 ORDERING — The Low/High Impact block must appear ABOVE the Edit/Write tool call in the same assistant turn, never below. As of v2.10.5, the PreToolUse hook structurally enforces this: between the previous Edit/Write and this one it looks for the [Rule 22] marker in a text block OR at the start of a line in a non-edit tool's input, and returns permissionDecision: deny if neither holds. Claude Code 2.1.280+ often does not persist visible text (it is saved as a paraphrased thinking block without the marker), so emit the block through Bash — cat <<'R22', then lines starting [Rule 22] ..., then R22 — which is recorded the moment it runs (no transcript wait) and is visible in the tool output. Visible text still counts, but may wait up to 40 s for the response to be written; agents without Bash (e.g. doc-updater) use visible text. A [Rule 22 · Scope] line is the post-edit check and does not count as the next edit marker. Retrying without the marker will deny again. Emit the block prospectively, not retroactively — the only valid path is marker-then-edit. Arguments for skipping ('conversation already covered it', 'docs-only edit', 'routine change', 'too trivial') are all invalid — see rules/change-decision-framework.md 'Ordering (required)' and 'Rationalizations that do not apply'. "
...
if [ "$KT_SESSION_STATE" = "true" ]; then
  MESSAGES="${MESSAGES}SESSION STATE — After the project/sub-project for this session is identified (by the PWD-based project match, or by what the user names in their opening message), locate SESSION.md at that project root (project root = nearest dir with CLAUDE.md/PROGRESS.md). If it exists with a non-empty '## Next session prompt' block: FIRST scan the user's ENTIRE opening message — not just its first line — for an 'aria-handoff: <project>/<sid>@<at>' token, because the operator may paste the opener with a sentence of their own in front of it. If a token is present, source bin/lib-session-state.sh and call kt_ss_ledger_token_locate <project_root> <token>: a result ending '|active' or '|pending' means the prompt is LIVE — resume it, then record the consumption, AND THE VERB DIFFERS BY RESULT KIND: for '|active' call kt_ss_ledger_consume_active <project_root> <your session id> <now UTC>, which demotes the prompt into '## Pending handoffs', marks that entry consumed, and clears the active slot; for '|pending' call kt_ss_ledger_mark_consumed passing the exact 'at' as the FIFTH argument (without it the match is by sid alone, and one sid can carry two timestamps). ⛔ Do NOT call kt_ss_ledger_mark_consumed on an '|active' result — it rewrites only '### ' headers and the active prompt has none, so it matches nothing, changes nothing and still returns 0; measured 2026-09-17, one real ledger held 142 entries of which 0 were ever marked consumed, and because kt_ss_ledger_prune reaps only a word-bounded 'consumed' that ledger was never prunable and reached 636,898 chars; a result ending '|archived' means it was DELIBERATELY taken out of the offer — say so and ask before resuming, never silently; an EMPTY result means the token names no prompt in this project — say so plainly and do NOT fabricate an entry; and if the located entry is already terminal, ⛔ SURFACE AND STOP rather than resuming, because a consumed prompt's work may already be shipped and irreversible. ⛔ AND, whether or not a token is present: any text the operator wrote alongside the pasted block is an INSTRUCTION and OUTRANKS the stored prompt — surface it and reconcile before executing, never drive past it. 'Before you start, see X' is a state-verify gate, not a preamble. If no token is present: if the user's opening message included the word 'handoff', open the session by executing that prompt directly (no confirmation); otherwise tell the user a saved resume prompt exists (state its lastEvent + age from the 'at' field) and ask whether to start from it (y/n). If the prompt's 'at' is older than session_stale_days (read from ~/.claude/aria-knowledge.local.md; default 7) days, do NOT present it as live — instead state its age and ask: still relevant? [resume / archive / keep]. 'archive' = move that entry under a '## Archived sessions' heading (atlas ignores it, same as '## Pending handoffs' and the legacy '## Prior sessions'); 'keep' = leave it as-is; 'resume' = execute it. Never auto-drop an aged entry — staleness prompts, it does not evict. ALSO: if a '## Pending handoffs' section (or legacy '## Prior sessions') holds entries still marked 'unconsumed', say how many and offer them alongside the active prompt — a SESSION.md may hold several still-valid next-session prompts, and one that is stored but never offered is lost in practice. If no such prompt exists, stay quiet. The 'in-progress' mark is now written automatically by the PostToolUse hook (post-edit-check.sh) on your first edit — do NOT write SESSION.md yourself here. Offer the resume once per session. "
fi
...
# Standing user rules (U-namespace) — ALWAYS in force, never opt-in. The user's own rules are
# at least as binding as the plugin's; anything less makes them a suggestion. Injects the rule
# HEADERS as an always-loaded index, with full bodies read on demand (a mature user-rules.md
# runs 50KB+ — too large to inject, while the titles are self-describing enough to bind).
# Same two-tier shape as the memory index. Absent file or zero rules → injects nothing.
UR_FILE="$KT_KNOWLEDGE_FOLDER/rules/user-rules.md"
if [ -f "$UR_FILE" ]; then
  UR_N=$(grep -c '^### U' "$UR_FILE" 2>/dev/null)   # -c always prints a count; no || fallback (it would append a 2nd line)
  if [ "${UR_N:-0}" -gt 0 ]; then
