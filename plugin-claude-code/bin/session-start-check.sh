...
MESSAGES="${MESSAGES}TASK BUDGET — If ~/.claude/aria-statusline-state.json exists (written by the aria-knowledge status-line meter), READ it to see your current context-window %, 5-hour, and 7-day plan-usage — do so when judging whether to keep going, and before /handoff, /wrapup, or compacting. (A UserPromptSubmit hook also auto-surfaces a warning when any metric crosses usage_alert_threshold, default 90%.) If that file does NOT exist, you do not see usage directly — Claude Code's UI shows the user; if strain symptoms appear (responses cutting short, deep session length, compaction warnings), surface them and offer options (finish the current atomic task, call /aria-knowledge:extract, trigger compaction, or continue). Either way, don't assume depletion or wrap up autonomously. "
