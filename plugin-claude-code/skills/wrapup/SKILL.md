...
⚠ **Tracked runtime drift — re-measured 2026-09-11, and the note this replaces was FALSE.** It said `plugin-antigravity` and `plugin-openai-codex` both *"carry this same step **without** the step-1 demote."* Measured (`kt_ss_ledger_add` occurrences per skill): **antigravity `wrapup` = 1 and `handoff` = 2 — both HAVE the demote, and both carry the `sessionId` conjunct this step just retired**; codex `handoff` = 2, the same; **only codex `wrapup` = 0 actually lacks the step.** ⇒ **Two different remediations, not one gap:** three sites need the conjunct dropped (antigravity ×2, codex `handoff`); one needs the step added (codex `wrapup`). Claude-Code is canonical this round. ⚠ **Re-derive this table whenever you cite it** — it is a per-runtime claim that goes stale at the next parity pass, and the note it replaces read as *measured*, which is precisely what stopped anyone re-measuring it. ⚑ The corrected reason above **does port**: `kt_ss_mark_inprogress` is byte-identical across claude-code / antigravity / cursor, and codex's copy differs only in its `.gitignore` block — all four pass the body through with the same bare `{ print }`. ⚠ **But that function is absent from the library parity set** (`kt_ss_ledger_add`, `kt_ss_ledger_prune`) **and has already drifted on one port**, so nothing guards the very behaviour the reason rests on.

**Before closing, check what this session left behind (two cheap reads, both report-only):**

1. **Rule 22 bypasses through the shell.** ⚠ There is no ledger to read any more. This step used to read `${TMPDIR:-/tmp}/aria-r22-bypass-<session_id>`, written by `pre-bash-write-check.sh` — retired 2026-08-26 as its only writer, because it decided from the command STRING rather than the resolved mutation TARGET and was therefore wrong in both directions. An instruction to read a file with no writer is worse than no instruction: it reports "no bypasses" from an empty file and reads as a clean result. Until a replacement exists, judge it directly — if you modified a tracked file through Bash this session, say so and say whether it carried a `[Rule 22]` assessment.
