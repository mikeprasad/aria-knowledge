...
**Before Step 0 — runtime self-correction.** Check whether the `Bash` tool is available in this session. If `Bash` is NOT available (you are running in Claude Cowork or another non-Code runtime), this invocation reached the wrong variant for the runtime. **Do not ask — redirect.** Announce it in one line, then use the `Skill` tool to invoke `aria-cowork:wrapup` with the same arguments the user provided, and do not run this skill's steps:

> ↪️ **Redirecting to `/aria-cowork:wrapup` — you invoked the aria-knowledge (Code) variant, but you are running in Claude Cowork or another non-Code runtime.**
>
...
1. **Demote a prior session's pickup.** If the existing SESSION.md holds an unconsumed entry carrying a **non-empty prompt block** — **whatever its `lastEvent`, and whatever its `sessionId`** — call `kt_ss_ledger_add` (**full-fidelity prompt — never collapsed, and the FENCED body ONLY — never the prior session's whole body; see `/handoff` 3f for why an unfenced column-0 `## ` orphans every ledger entry after it**) FIRST, THEN write. This mirrors `/handoff` step 3f exactly, for the same reason: the full rewrite below replaces `## Where we left off`, `## Next session pickup` and `## Next session prompt` — precisely where `/handoff` puts a pickup. Without the demote, wrapping up one session **destroys** another session's handoff. **Skip the demote only for a FRESH `in-progress` marker whose body is just `(session in progress)`** — that is the one state in which the stored entry would be empty. ⛔ **A `lastEvent: in-progress` marker that carries a real prompt MUST be demoted.** `kt_ss_mark_inprogress` rewrites only the front-matter keys and passes the body through with a bare `{ print }`, so such a marker routinely still holds the previous session's entire `## Next session prompt` — and `post-edit-check.sh` creates that state automatically whenever a second session edits in a project holding a handoff. The rationale this replaces asserted the opposite and its effect was to overwrite live pickups; do not restore it, and do not restate it in order to explain it — a test asserts its exact wording is absent from this section. ⛔⛔ **The gate does NOT test `sessionId`, and that is deliberate.** A front-matter `sessionId` names whoever last **touched** the file, not whoever wrote the **body**: `post-edit-check.sh` stamps the current session's id onto the front-matter while `kt_ss_mark_inprogress` passes the body through, so **your own id sitting over another session's prompt is the routine state, not an edge case** (`tests/repros/session-state.sh` block O asserts exactly that). `bin/lib-session-state.sh` records the same finding for the ledger key — *"the key can hold a value a HOOK wrote rather than the author."* ⚠ **`by:` does not rescue it either:** that field IS preserved by `kt_ss_mark_inprogress` (its awk rewrites five keys and passes the rest through), but it is **person-granular**, and this state is produced by the *same person in a second session*. ⇒ **A prompt you wrote yourself and a prompt a hook re-badged as yours are indistinguishable here, so both are demoted.** A pending entry you did not want is one deletion; an erased prompt is git archaeology. ⚠ **Consequence, chosen rather than incidental:** if you ran /handoff earlier in **this** session, /wrapup demotes **your own** pickup into `## Pending handoffs` rather than erasing it — delete the pending entry by hand if you meant to discard the prompt. (The *"there is no next-session prompt to retain"* note below is about not adding a **new** entry for the wrapped session; it is not a claim that no prompt exists.) ⚠ In a **multi-author** project where `by:` differs between the two sessions the two cases *are* distinguishable — the gate still does not test it, because `by:` cannot separate two sessions of one author, which is the generator here. ⚠ **`<prior sessionId>` may be a HOOK ARTIFACT — annotate it when it is.** If the value you are about to pass equals **this** session's id while the prompt block is not one you wrote, the front-matter was stamped by `post-edit-check.sh` and the true author is **not recoverable from the file**. Prefix `<prior currentFocus>` with `⚠ sid is a post-edit-check.sh HOOK ARTIFACT, not attribution —` so the entry does not read as yours. ⛔ **It goes in `focus`, never in the sid** — the sid is what `kt_ss_ledger_mark_consumed` (`^### .*<sid>`) and `is_entry_header` key on, and a ledger checker captures everything before the first `·` as the id. (A human reached for this exact shape unprompted, twice, before it was specified.)
...
✅ **Runtime parity: CLOSED 2026-09-11, and ENFORCED rather than noted.** All four port skills (`plugin-antigravity` wrapup + handoff, `plugin-openai-codex` wrapup + handoff) now carry this step with the same body-only gate. ⛔ **Do NOT maintain a per-runtime census here again.** The prose note this replaces was wrong three times — false as shipped (it said the ports lacked the demote; antigravity had it, and had the bug), understated after its first correction (it said three conjunct sites, when the two spellings summed to **five**), and would have gone false again the moment the ports closed. The invariant is now asserted by **`tests/repros/wrapup-demotes-before-rewrite.sh` block P**, which scopes itself to every port carrying `kt_ss_ledger_add` and fails on any of the three retired spellings. ⚑ Two things the guard cannot cover: `kt_ss_mark_inprogress` is absent from the library parity set (`kt_ss_ledger_add`, `kt_ss_ledger_prune`) and has drifted on one port — harmless today, since all four pass the body through identically, but unguarded; and codex `wrapup` still instructs reading a retired R22-bypass ledger that has no writer, which is a separate defect, deliberately not bundled here.
