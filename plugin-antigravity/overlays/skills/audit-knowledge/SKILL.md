...
**Age annotation and stale marker:** For each idea file, compute age as `(today - idea date)` in days. Derive the idea date as follows: (1) read `date:` from YAML frontmatter; (2) if missing or malformed, fall back to the `YYYY-MM-DD` prefix of the filename. Annotate each entry with its age (`filed N days ago`). Read the staleness threshold from `~/.gemini/antigravity/aria-knowledge.local.md` (`ideas_staleness_threshold_days`, default 7) via `config.sh` or fallback. When `age > threshold`, append a `[STALE — still relevant?]` marker to the entry and escalate its visual weight in Step 6 (place stale entries first within the Pending Ideas section, and prompt explicitly for Accept/Reject/Defer rather than allowing implicit Defer).
...
- **User memory `feedback_*.md`** — for personal/working-style rules that span projects (matches the existing feedback-memory pattern under the active project's `~/.gemini/antigravity/transcripts/{cwd-encoded}/memory/` directory)
...
> 4. **Clear** — ledger-clear all snapshots without reviewing (v2.15.2+: bodies removed but a REMOVED.md ledger records session-ids + timestamps + pointers to the transcript file)
...
- If **Clear** → apply the **ledger-clear pattern** (v2.15.2+):
  ...
  2. Write `{knowledge_folder}/archive/audit-{date}/pre-compact-captures/REMOVED.md` with frontmatter (`audit_date`, `removed_count`, `canonical_source: ~/.gemini/antigravity/.last-transcript-path`) + a per-snapshot list of filename + session-id + capture-timestamp + canonical-jsonl-pointer. See the ledger schema at the end of this section.
  3. After the ledger is written, `rm` the snapshot `.md` files from `intake/pre-compact-captures/`. Bodies are *derived copies* of the Antigravity conversation transcript — canonical preservation is at the path stored in `~/.gemini/antigravity/.last-transcript-path`.
...
Pre-compact snapshots are *derived copies* of the Antigravity conversation transcript at the path stored in `~/.gemini/antigravity/.last-transcript-path`. Their bodies are not preserved in the archive — only a ledger of what existed is preserved. The body's canonical source is the Antigravity transcript file (at the cached path) until the conversation ends or the file is rotated.
...
audit_date: YYYY-MM-DD
removed_count: <N>
canonical_source_pattern: ~/.gemini/antigravity/.last-transcript-path (points to current transcript.jsonl)
note: |
  Pre-compact snapshots are derived copies of the Antigravity conversation transcript.
  Bodies are not archived here; the canonical source retains them until the conversation
  ends or is archived. If a snapshot needs recovery, read the transcript at the cached path.
---

# Removed pre-compact snapshots — {audit-date} audit

- {filename} | session {session-id} | captured {capture-iso8601} | disposition: clear|approved|rejected | reason: {one-line} | canonical: {transcript-path-from-cache}
- ...
```
...
- **Transcript** — `~/.gemini/antigravity/.last-transcript-path` points to the current conversation's `transcript.jsonl`. This is the equivalent of Claude Code's per-cwd memory + plan files combined — all of the agent's prior actions, decisions, and reasoning live here.
- **Artifact directory** — `~/.gemini/antigravity/.last-artifact-dir` points to the current conversation's artifact directory. Contains screenshots, generated images, and any other artifact the agent has produced.

If either cache file doesn't exist, the pre-invocation hook hasn't fired yet — stop here with: *"No conversation state cached. Let the agent respond at least once, then re-run /audit-knowledge."*

### 2a: Scan the Transcript

```bash
TRANSCRIPT=$(cat ~/.gemini/antigravity/.last-transcript-path 2>/dev/null)
if [ -z "$TRANSCRIPT" ] || [ ! -f "$TRANSCRIPT" ]; then
  echo "Transcript cache empty or stale: ${TRANSCRIPT:-<none>}"
  exit 1
fi
```

Read the transcript jsonl in chunks. Look for the same extractable patterns the canonical version looks for in memory + plan files: insights, decisions, rules, approaches, references, and pattern repetitions. The transcript holds all of these — the agent's reasoning chain across the conversation is the data source.

### 2b: Scan the Artifact Directory

```bash
ARTIFACTS=$(cat ~/.gemini/antigravity/.last-artifact-dir 2>/dev/null)
if [ -n "$ARTIFACTS" ] && [ -d "$ARTIFACTS" ]; then
  find "$ARTIFACTS" -type f -name '*.md' -o -name '*.txt' 2>/dev/null
fi
```

Inspect any markdown/text artifacts. Plan documents, drafted ADRs, or notes the agent has generated live here — these are direct extraction candidates.

### 2c: Continue to canonical audit logic

The rest of the audit pipeline (compute trigger state, route to appropriate surfaces, present accept/reject submenus) is port-agnostic — the only Antigravity-specific change is *where* knowledge is sourced (transcript + artifacts vs Claude Code's memory + plans dirs).
...
Also check project-level GEMINI.md and docs files in the current working directory for already-captured knowledge.
...
**Files scanned:** X transcript entries, Y artifacts

### Summary
- Category A (already captured): X items
- Category B (Antigravity-session-specific): Y items
- Category C (worth extracting): Z items
...
- **Source:** transcript segment or artifact file path
...
If none: emit "**Category C Items:** 0 — all scanned items were Category A or B."
...
- **Evidence:** [which backlog entries / transcript segments point to this]
...
- **Trigger:** count=N threshold=T days=D cadence=C — (which fired: count-tier|days|user-invoked)
- **Result:** No new items — [X transcript segments all Category A, Y artifacts Category B, backlogs empty OR K entries all cleared as already-captured/stale]
...
Safe to call even if Step 7a's `kt_batch_begin` didn't succeed (e.g., jq missing) — the function just removes the manifest file if it exists. If the audit errors out before reaching Step 8b, the session-start check's stale-manifest auto-clear (30-minute threshold) recovers on the next session start so stale manifests don't silently suppress Rule 22 on unrelated edits.
...
- **Check project docs thoroughly** — knowledge is often already captured in project-level GEMINI.md, PROGRESS.md, or docs/ folders
- **Convert relative dates** — if a transcript or artifact references "last Thursday", convert to the actual date
- **Stale transcript segments are not Category C** — outdated project status doesn't need extraction, it needs cleanup
