...
├── aliases.md               # Tag aliases (since v2.16.0) — /index and /context resolve these to canonical tags
...
│   ├── intake-doc.md        # Frontmatter schema for captured docs (used by /intake)
...
│   └── subagent-captures/   # Subagent return snapshots (when subagent_capture is on)
├── logs/                    # Operational logs — audit dates and results
│   ├── knowledge-audit-log.md
│   ├── config-audit-log.md
│   ├── prospect/            # Per-run /prospect logs (<date>-<scope>-<slug>.md)
│   └── retrospect/          # Per-run /retrospect logs (<date>-<scope>-<slug>.md)
...
│   └── overbuild-patterns.md   # Canonical over-build pattern library (powers the overbuild lens in /prospect, /retrospect, /readiness-audit)
...
├── pm-reviews/              # Dated morning PM reviews (generated by /aria-assist; see pm_schedule_time)
