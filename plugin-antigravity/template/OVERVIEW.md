...
A batch manifest is a JSON file at `~/.gemini/antigravity/active-batch.json` declaring expected operations for a bulk task. The `pre-edit-check.sh` hook reads it and emits a compressed directive for declared-low-impact matches; everything else falls through to the full CHANGE DECISION CHECK.
