...
During `/audit-knowledge`, idea files older than `ideas_staleness_threshold_days` (default 7, configurable in `~/.gemini/antigravity/aria-knowledge.local.md`) are tagged `[STALE — still relevant?]` and require an explicit Accept / Reject / Defer decision — you can't implicitly defer a stale idea.
