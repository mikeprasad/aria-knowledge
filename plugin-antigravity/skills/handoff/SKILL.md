...
**Multi-session ledger (several valid prompts can coexist — nothing is ever lost):** if the existing SESSION.md carries a **non-empty prompt block** — whatever its `lastEvent` — and its `sessionId` differs from this session's (or is absent — treat an unidentifiable handoff as *someone else's*), DEMOTE it before overwriting the active slot. Source `bin/lib-session-state.sh` and, reading the prior file's values first, call:
- `kt_ss_ledger_add "{project_root}" "<prior sessionId>" "<prior at>" "<prior currentFocus>" "<prior nextAction>" "<prior next-session-prompt>"` — moves the prior active entry into `## Pending handoffs` (newest-first). **Pass the prompt at FULL fidelity — do not collapse it to one line.** An unconsumed prompt is still-valid work; collapsing it degrades a mandate nobody has used yet. ⛔ **Pass the FENCED prompt block** — the ```-delimited body sitting under `## Next session prompt`, *without* that heading — and never the prior session's whole body. ⚠ **The terminator declares the boundary for the SHELL HELPERS ONLY.** `kt_ss_ledger_*` boundary on `<!-- aria:entry-end -->`, so an unfenced column-0 `## ` inside a stored prompt is harmless to them — but the SESSION STATE directive and `tools/check-session-ledger.py` are **heading-scoped**, and to those readers such a line ENDS the ledger section and orphans every entry after it. Measured on `cs/SESSION.md` 2026-09-09: one demoted entry carried all three of `## Where we left off`, `## Next session pickup` and `## Next session prompt` — the signature of a caller who passed the whole body — and **75 downstream entries fell outside the ledger**. A fenced body is safe for both readers; nested fences inside it are preserved as-is.
