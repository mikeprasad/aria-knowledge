...
This is an **alias skill**. The canonical implementation lives at `plugin/skills/audit-share/SKILL.md` within the aria-knowledge plugin. Invoking `/share-audit` and `/audit-share` produces identical behavior — this alias exists only to accommodate users who prefer the inverted "share-audit" phrasing.

## Execute

Read `plugin/skills/audit-share/SKILL.md` (relative to the aria-knowledge plugin root — resolve via `${CLAUDE_PLUGIN_ROOT}/skills/audit-share/SKILL.md` if available, otherwise locate it within the installed plugin tree) and follow every step in that file exactly, passing through any arguments the user provided to this alias.
