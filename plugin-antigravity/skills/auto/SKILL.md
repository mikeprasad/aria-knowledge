...
- **D7 — The judgment ledger.** Any decision that could not be **Validated** (checked
  ...
  **Confirmed after** (what was predicted actually held once built) is logged. All four
  ...
  `knowledge_folder` from `~/.gemini/antigravity/aria-knowledge.local.md`. Create `logs/auto/` lazily.
...
- **`tickets`** — tracker-bound. Work selection comes from the connected tracker by
  ...
  **Resolving the tracker — never hardcode a vendor.** Probe at runtime for a connected
  ...
  helper API (ADR-015). If `ticketing_plugins` is set in `~/.gemini/antigravity/aria-knowledge.local.md`
...
1. **Resolve + read.** Read `~/.gemini/antigravity/aria-knowledge.local.md` and extract `knowledge_folder` (same resolution the `/rules` skill uses). Read `{knowledge_folder}/rules/user-rules.md` if it exists. It is **optional and user-owned** — it may be absent (pre-v2.8.1 setups) or present with no `### U` rules yet. If absent, or present but empty of rules, treat it as **no standing user rules** and continue — a missing file is never a stop.
...
**The preflight commit gate — satisfy it, never route around it.** A user may configure `pre-commit-preflight-check.sh` to DENY commits (`preflight_gate: deny`, a `preflight_deny_repos` substring matching the target repo, or a `preflight_deny_paths` glob matching a staged path — all read from `~/.gemini/antigravity/aria-knowledge.local.md`). Under an autonomous arc that is not an obstacle to work past: **before the first commit of the arc that the gate would deny, actually run `/preflight`** on the work about to be sealed, then commit. The marker is session-scoped, so one genuine run clears the gate for the remainder of the arc — this costs one checklist, not one per task.
...
⚠ `ls ~/.gemini/antigravity/scheduled-tasks/` is **not** a registration oracle — de-registering a task leaves
