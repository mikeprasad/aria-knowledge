...
## Resolve config
1. Read `~/.gemini/antigravity/aria-knowledge.local.md`. The roster is `projects_list` (comma-separated
...
1. Read `~/.gemini/antigravity/aria-pm-facts.json` (the launchd wrapper `pm-morning-run.sh` pre-generates it).
   If missing/stale AND Bash is available (a manual run), regenerate it:
   `sh ${CLAUDE_PLUGIN_ROOT}/bin/pm-collect.sh ~/.gemini/antigravity/aria-pm-facts.json`. In headless runs Bash is disallowed —
