...
- **STITCH path:** `{project_root}/STITCH.md` by default; override with `projects_groups[<tag>].stitch_path` if set in `~/.gemini/antigravity/aria-knowledge.local.md`. **Skip STITCH entry silently** if the project tag is NOT in `projects_groups` (no group registered — a single undifferentiated codebase with no contract→multi-client seam, so `/stitch` doesn't apply; note a monorepo with such a seam SHOULD be registered as a group and would get a STITCH entry).
