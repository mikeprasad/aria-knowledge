...
- If **Clear** → apply the **ledger-clear pattern** (v2.15.2+):
  ...
  2. Write `{knowledge_folder}/archive/audit-{date}/pre-compact-captures/REMOVED.md` with frontmatter (`audit_date`, `removed_count`, `canonical_source: ~/.gemini/antigravity/transcripts/...`) + a per-snapshot list of filename + session-id + capture-timestamp + canonical-jsonl-pointer. See the ledger schema at the end of this section.
  3. After the ledger is written, `rm` the snapshot `.md` files from `intake/pre-compact-captures/`. Bodies are *derived copies* of Claude Code's per-session transcript log — canonical preservation is at `~/.gemini/antigravity/transcripts/{cwd-encoded}/{session-id}.jsonl` (until Claude Code rotates that log).
...
Pre-compact snapshots are *derived copies* of Claude Code's per-session transcript log at `~/.gemini/antigravity/transcripts/{cwd-encoded}/{session-id}.jsonl`. Their bodies are not preserved in the archive — only a ledger of what existed is preserved. The body's canonical source is the Claude Code transcript log (until Claude Code rotates it).
...
audit_date: YYYY-MM-DD
removed_count: <N>
canonical_source_pattern: ~/.gemini/antigravity/transcripts/{cwd-encoded}/{session-id}.jsonl
note: |
...
- {filename} | session {session-id} | captured {capture-iso8601} | disposition: clear|approved|rejected | reason: {one-line} | canonical: ~/.gemini/antigravity/transcripts/.../{session-id}.jsonl
- ...
```
...
Read all `.md` files in `~/.gemini/antigravity/transcripts/` memory directories for the current project (excluding `MEMORY.md` itself).
