...
description: "Audit a surface against a checklist to answer 'is it clean/legal/consistent to ship for THIS event?' — the recurring sibling of /foundational-review. Parallel exploration per surface (source/build · licensing/hygiene · docs/tooling) → controller re-verification of every load-bearing agent claim with a correction trail → tiered findings (Tier 0 blockers / High / Medium / Low) each with a verified Evidence cell → conceptual observations → phased remediation plan with owners → end-to-end verification recipe → gates for genuine decisions. Read-only probes only; mandatory artifact diff-check before staging (a prior agent-overwrote-a-generated-file incident). Findings are NOT a shipping list — they triage to phases. No irreversible-decision anchor required (contrast /foundational-review). Triggers: '/readiness-audit <scope-root> --for \"<event>\"', '/audit-ready', 'readiness audit', 'release-readiness audit', 'is this ready to ship/publish/hand over', 'public-release audit'."
---
...
1. Read the **"Companion format: the readiness audit"** section of the canonical process doc, preferring a user copy when present:
   - If `<knowledge_root>/approaches/foundational-review-chain.md` exists (resolve `<knowledge_root>` from `~/.gemini/antigravity/aria-knowledge.local.md`), read THAT.
