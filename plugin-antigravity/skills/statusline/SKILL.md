...
Then write `~/.gemini/antigravity/settings.json` with the `statusLine` block merged in, **preserving all existing keys** (read the current JSON, add/replace only the `statusLine` key, write the whole object back — valid JSON, 2-space indent). The block:
...
- Script: ~/.gemini/antigravity/aria-statusline-meter.sh
- Wired in: ~/.gemini/antigravity/settings.json (backup at settings.json.aria-bak)
...
1. Read `~/.gemini/antigravity/settings.json`. If it has no `statusLine` key, report "No status-line meter is configured." and stop.
...
4. Offer to delete the staged script + snapshots: *"Also delete the meter script (`~/.gemini/antigravity/aria-statusline-meter.sh`) and its per-account usage snapshots (`~/.gemini/antigravity/aria-statusline-state-*.json`)? (y/n)"* — on `y`, `rm -f` the script plus the `aria-statusline-state-*.json` glob (and the legacy `aria-statusline-state.json` if present), plus any `/tmp/aria-usage-warn-*` band-state files.
5. Confirm: "Status-line meter removed. Backup at ~/.gemini/antigravity/settings.json.aria-bak." (The usage-alert hook stays registered but is now a silent no-op with no snapshot to read.)

---

## Status

1. Report whether `~/.gemini/antigravity/aria-statusline-meter.sh` exists.
2. Read `~/.gemini/antigravity/settings.json` and report whether `statusLine` is set and what `command` it points at (and whether that's the aria-knowledge meter).
