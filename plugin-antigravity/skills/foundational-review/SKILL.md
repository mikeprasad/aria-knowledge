...
1. **Load the canonical chain.** Read the canonical process doc, preferring a user copy when present:
   - If `<knowledge_folder>/approaches/foundational-review-chain.md` exists (resolve `<knowledge_folder>` from `~/.gemini/antigravity/aria-knowledge.local.md`), read THAT — a user may keep a richer, project-specific copy there.
