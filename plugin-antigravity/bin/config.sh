...
KT_CONFIG="${KT_CONFIG:-$HOME/.gemini/antigravity/aria-knowledge.local.md}"
KT_CONFIGURED=false
KT_CONFIG_ERROR=""
...
kt_resolve_account() {
  ...
  # Tier 4: CLI / VS Code / API-key -> ~/.gemini/antigravity.json (v2.24.2 behavior, unchanged)
  _kra_uuid=""; _kra_email=""
  if command -v jq >/dev/null 2>&1 && [ -f "$HOME/.gemini/antigravity.json" ]; then
    _kra_uuid=$(jq -r '.oauthAccount.accountUuid // empty' "$HOME/.gemini/antigravity.json" 2>/dev/null)
    _kra_email=$(jq -r '.oauthAccount.emailAddress // empty' "$HOME/.gemini/antigravity.json" 2>/dev/null)
  fi
...
# declare an active batch by writing ~/.gemini/antigravity/active-batch.json. The pre-edit
...
KT_BATCH_MANIFEST="$HOME/.gemini/antigravity/active-batch.json"
...
kt_batch_begin() {
  ...
  mkdir -p "$HOME/.gemini/antigravity" 2>/dev/null
