...
# (~/.gemini/antigravity/aria-statusline-meter.sh) and points settings.json -> statusLine at
...
if command -v jq >/dev/null 2>&1; then
  ...
  # only when ~/.gemini/antigravity.json is the real session account.
...
if command -v jq >/dev/null 2>&1; then
  ...
  _state="$HOME/.gemini/antigravity/aria-statusline-state-${_key}.json"
  _tmp="$HOME/.gemini/antigravity/.aria-statusline-state.$$.tmp"
