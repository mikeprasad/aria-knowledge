...
if command -v jq >/dev/null 2>&1; then
  ...
  # account field) but from ~/.gemini/antigravity.json, which updates on every /login switch.
  # Used to scope the state snapshot per-account and to display the email.
  if [ -f "$HOME/.gemini/antigravity.json" ]; then
    acct_email=$(jq -r '.oauthAccount.emailAddress // empty' "$HOME/.gemini/antigravity.json" 2>/dev/null)
    acct_uuid=$(jq -r '.oauthAccount.accountUuid // empty' "$HOME/.gemini/antigravity.json" 2>/dev/null)
  fi
else
