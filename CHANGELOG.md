...
**The post-edit `[Rule 22 · Scope]` line no longer satisfies the next edit's pre-edit gate.** Found by 2.54.1's live check: a `doc-updater` Write with no assessment was allowed because the line above it was the subagent's answer to the previous edit's scope check. Every matcher accepted `[Rule 22 · <anything>]`, while the framework names `· Scope` as the post-edit line and ADR 036 requires an assessment before every edit — so an agent answering each scope check never had to assess. Live where visible text is saved (subagents); mostly latent on the Claude Code 2.1.280 main thread.

All six matchers now reject a `· Scope` variant: `pre-edit-check.sh` (text and line-start-in-tool-input), the carrier recorder (a Scope-only heredoc records no carrier), codex's `has_rule22_marker` (codex prompts for a Scope line after every edit), codex's unwired `pre-edit-check.sh`, and antigravity by regeneration. Every post-edit form measured in local transcripts is excluded — `Scope`, `scope`, `SCOPE`, `Scope check`, `Scope-check`, and a double space — while a different word such as `· Scoped change`, and every pre-edit form (`[Rule 22]`, `· Planning`, `· Batch N/M`, `· Implementation`), still counts. The exclusion is a lookahead rather than an allow-list because `· Implementation` is heavily used and undocumented. ⚑ The first design (`(?!(?i:scope)\])`) still accepted `Scope check` and `Scope-check`, and a lookahead after an optional `\s*` let backtracking hide `Scope` behind a space; both were caught by tests before commit. The deny message, the always-on digest and the framework now say that a Scope line does not count.

**The plugin test suite no longer touches the real `$TMPDIR`.** `plugin-claude-code/tests/run.sh` did not isolate it, so tests wrote hook state there and `test-external-fetch-gate.sh`'s `ef_reset` ran `rm -f ${TMPDIR:-/tmp}/aria-extfetch-*` — deleting every live session's external-fetch cooldowns and breaker counters on each run (28 files leaked per run, measured). The runner now exports an isolated `TMPDIR`.

New repros: `tests/repros/r22-scope-line-not-a-marker.sh` (15 controls on real transcript bytes) and `tests/repros/plugin-suite-leaves-real-tmpdir-alone.sh` (runs the real suite against a sandbox inherited `TMPDIR` and asserts nothing in it is created or removed). New codex test `test_scope_line_is_not_a_pre_edit_marker`. The previous code fails exactly the declared set; all eight named mutations are caught. Codex ships as `2.46.5-codex.0` (it is not at canonical parity, so it keeps its own version line).
...
**The fix is that a prompt now carries its own identity, rather than borrowing it from a field anyone may overwrite.** `/handoff` emits a provenance token as the first line inside its opener fence — project, session id and timestamp together — and every consumer prefers that token over the front-matter when the two disagree. The structural check gained a warning for exactly that disagreement, which previously had no detector at all. The token is tested **before** the fence-skip, deliberately: it lives inside a fence, so a reader that skips fences first can never see it.

**`kt_ss_ledger_mark_consumed` was matching session ids as a REGEX, and could consume the wrong entry.** The matcher interpolated the id into a pattern, so a metacharacter matched something other than itself and a prefix matched a longer id. It is now a literal `index()` test against the header's first field, with an optional timestamp conjunct so a session id reused across two entries resolves to exactly one.

**An ordering guard, because nothing compared timestamps at all.** Measured: zero comparisons of the incumbent's `at` existed in the library. The front-matter is single-valued and last-writer-wins across concurrent sessions, so an older session's close-out silently regressed a newer session's state. A session that would overwrite a newer incumbent now self-demotes — its work lands in `## Pending handoffs` and the incumbent's front-matter is left untouched. ⛔ **The fail direction is the conservative one and is load-bearing**: a missing, absent or unparseable stamp yields today's behaviour, never a block. A new guard must not invent a new way to lose a write.

⚑ The stamp shape gate accepts **second-precision `Z` only**, and the first draft was over-engineered against the wrong population — it argued for minute-precision support by citing a stamp that was an *entry header* when this guard reads *front-matter*, and one that the same session had already renamed away. Re-measured on the correct population: 8 of 8 tracked ledgers carry second-precision. A live local-offset stamp does exist in the corpus, which is why non-`Z` is rejected rather than compared: it sorts wrong against a `Z` stamp while looking entirely reasonable.

**`/handoff resume` and a picker**, so a stored prompt can be resumed without being hunted for. ⚠ The picker's first cap was 5 rows, which **hid 39 of 44 fresh candidates** — the fix was rendering, not policy: one-line rows show all 44 in less space than 5 multi-line ones.

⚠ **Reported and deliberately NOT fixed: the codex port references `kt_ss_ledger_add` twice in its `handoff` skill and once in `wrapup`, and ships no ledger functions at all.** Those calls cannot succeed there today, so the demote silently does not run and the rewrite destroys a prior pickup. Porting this arc's guard there would have added a *second* reference to a helper the port does not ship — more correct-looking, no more runnable. The defect predates this arc; fixing it means either shipping six helpers or removing the clauses, both real decisions with their own blast radius, and neither belongs inside a parity pass.

⭐ **A hygiene finding this release also fixes, and a gap it exposes.** Five unpushed commit messages named a client project directory, which this repo's fatal public-hygiene gate correctly refuses to publish. Rewriting them re-SHAed 22 commits and left 38 citations across 8 documents pointing at objects that no longer exist; all 38 were repointed, length-preserving, and every replacement verified to resolve against a firing negative control. The rewrite ran inside an isolated linked worktree rather than the shared one, so a peer session's uncommitted edit never had to be moved. ⛔ **The gap: that gate scans `origin/main..HEAD`, so commit messages on any other local branch are invisible to it** — the backup branch created during this work held all five violations and the gate read clean. It was deleted rather than kept.
...
v2.48.2 widened the entry-type conjunct — from ``an unconsumed `handoff` entry`` to *an unconsumed entry carrying a non-empty prompt block, whatever its `lastEvent`* — and carried `(different or absent sessionId)` forward verbatim. `git log -S` returns exactly one commit for the survivor: the original demote fix. So the clause stated two disagreeing tests two sentences apart, a trigger that vetoed on a matching `sessionId` and an "only if" override saying a marker carrying a real prompt MUST be demoted.

**Why `sessionId` is the wrong field, now stated in the clause instead of left implicit.** It names whoever last *touched* the file, not whoever wrote the *body*: `post-edit-check.sh` stamps the current session's id onto the front-matter while `kt_ss_mark_inprogress` passes the body through, so your own id sitting over another session's prompt is the routine state rather than an edge case. `bin/lib-session-state.sh` had already recorded the same finding for the ledger key — *"the key can hold a value a HOOK wrote rather than the author"* — so this propagates a ruling that already existed one layer down. The library is unchanged; it was right.

`by:` does not rescue it either. That field *is* preserved by `kt_ss_mark_inprogress`, and it is in the canonical session contract — but it is person-granular, and the state above is produced by the same person in a second session. Both front-matter identity fields fail to discriminate, for different reasons. A prompt you wrote yourself and a prompt a hook re-badged as yours are indistinguishable at the gate, so both are demoted. One consequence is stated as chosen behaviour rather than left as a side effect: `/handoff` then `/wrapup` in one session now demotes your own pickup into `## Pending handoffs` instead of erasing it.

**Nothing caught it because a control asserted it.** Assertion `B` in `tests/repros/wrapup-demotes-before-rewrite.sh` *required* the retired phrase, and its failure message argued to restore it — so the suite was green over a self-contradicting clause, 16 passed and 0 failed. It is re-pointed, not deleted: absence of the retired conjunct is asserted with an exact string, and the positive arms key on ``whatever its `sessionId` `` rather than the bare word, because the corrected clause necessarily explains `sessionId` and a bare-word arm would be satisfied by the prose.

**Also closed: a demoted entry's attribution.** `kt_ss_ledger_add` takes the prior `sessionId` from the front-matter, which may be the demoting session's own id, so a correctly-demoted entry was misattributed to whoever demoted it. Both clauses now instruct annotating that in `focus` — never in the sid, which is what `kt_ss_ledger_mark_consumed` and `is_entry_header` key on. Two operators had already hand-written this warning, weeks apart, before it was specified.

**All four ports closed, and parity is now enforced rather than noted.** The census had to be corrected twice: the first figure counted one spelling of the conjunct and missed the ledger clauses, which use the other, so five sites rather than three. And `plugin-openai-codex` was two generations behind, not one — its handoff still keyed on ``an unconsumed `handoff` entry`` and on ``has `lastEvent: handoff` and its `sessionId` differs``, the exact string this suite's own assertion `H2` asserts must be absent. Codex `wrapup` had no demote step at all under a "Full rewrite", i.e. the original data-loss path still live in that runtime, and it ended with *"Unconsumed handoffs survive at full fidelity"* — the sentence assertion `D` guards against because it is true of ledger entries and false of a handoff in the active body.

The per-runtime prose note that was supposed to track this was wrong three times: false as shipped, understated after its first correction, and would have gone false again the moment the ports closed. Its job moves to a new assertion, block `P`, which scopes itself to every port carrying `kt_ss_ledger_add` and fails on any of the three retired spellings. Keying on the demote's presence rather than on section structure matters twice — the six wrapup/handoff files have genuinely distinct layouts, so slicing them by heading would silently examine nothing, and a port is covered automatically the moment it gains a demote, which is how codex `wrapup` entered the set. `plugin-claude-cowork` is correctly outside it: that runtime uses in-memory dedup rather than the ledger.

Ten mutations, every expectation pre-declared and every count matched. The two that carry the weight: reverting handoff's 3f line alone reddens its own assertion and leaves the ledger clause's green, so a partial fix across the two sites is detectable; and dropping a port's demote reddens block `P`'s anti-vacuity arm, so the per-port loop cannot pass having examined nothing. Suites 498 → 507 and 332 → 332, both green.

Two adjacent defects are recorded and deliberately not bundled: `kt_ss_mark_inprogress` is absent from the library port-parity set and has already drifted on one port — harmless today, since all four pass the body through identically, but unguarded; and codex `wrapup` still instructs reading a retired R22-bypass ledger that has no writer, which reports "no bypasses" from an empty file.
...
The obvious fix was to raise the window until the corpus fits. This release does not, and the reason is narrower than it first appeared. `/audit rules` Step 7 tells an author to verify a new lead fits the budget and `bin/check-rule-lead-bytes.sh` checks it at promotion, so the two numbers are paired and must move together; and the pairing is measurably shaping authoring — the seven newest rules all land between 209 and 234 bytes against a 240 budget. ⚠ **What it is not: an independently-ruled budget.** The gate holds 240 because the generator does (its own wording is descriptive), and it exists to fix a chars-vs-bytes unit error. So widening the window remains available as a coordinated two-constant change, and a test now asserts that relationship rather than the literal. What this release fixes is the *rendering*, which was severing claims at any window.
...
**And the gate and the generator now share ONE definition of a rule's lead — they did not.** Reported by KW Low: `bin/check-rule-lead-bytes.sh` appeared to measure 24 bytes for every rule. The symptom reproduced exactly; the cause was not the one reported, and the difference changes the fix. The script extracts the first non-empty paragraph correctly — on a 27-rule corpus it reads real leads from 53 to 565 bytes. What it lacked was any concept of a **metadata paragraph**. The generator had exactly one (`**Origin:`); the gate had none.

So on an `**Origin:`-first rule — the shape `/audit rules` Step 7.1 tells every author to write — the generator rendered the lead while the gate measured the provenance block. Measured: 71 bytes against 62, no error on either side. And a rule leading with any other marker (`**Last updated:`) had that marker rendered as its entire always-on body, with the gate reporting `OK`.

Both files now carry the **same** marker set — `Origin`, `Last updated`, `Status`, `Superseded` — skipped when they lead, ending the lead when they follow. Two new gate outcomes: `NOLEAD U<n>` (non-zero exit) when a rule has no operative paragraph at all, and a non-fatal `UNKNOWNLABEL U<n>` when a lead opens with an unrecognised bold label. ⛔ `Why` and `How to apply` are **deliberately absent and must never be added** — they are body structure per Step 7.1, so skipping them would discard real rule content; an unlisted label is reported rather than guessed.

⛔ **A metadata-only rule was rendering a dangling separator, and branch (iv) never covered it.** `flush()` computed `length(p)` on an empty lead = 0, which satisfies `length(p) <= window`, so **branch (i)** fired and emitted `- **U1 — Title** — ` with nothing after it. Branch (iv)'s guard is `length(p) > ceiling` and an empty lead is the smallest possible value, so it was unreachable. Present at HEAD before this release, since `**Origin:` was already skipped. Now an explicit first branch renders title-only, with no separator.

**The class-close is a test, not a shared implementation.** A single shared awk program was considered and rejected: the gate must stay standalone-runnable by path (Step 7.2 invokes it directly), and a `-f` dependency is the coupling class that once shipped `session-start-rules.sh` without its own library — the hook exited 1 emitting zero bytes. Instead `[UD13]` asserts the two sides select the **same paragraph**, compared against **each other** rather than against literals; `[UD19b]` asserts the two marker sets are identical. Suite 315 → 331 assertions. Seven mutations, each RED for its **named** control and each restored from a byte backup verified with `cmp`.

⚑ **One of those seven came back GREEN first time, and it was the mutation that was wrong, not the guard.** Adding `Why` to the skip set changed nothing observable because no fixture led with `Why` — an unfaithful mutation reads exactly like a working guard. A `Why`-led fixture was added and the mutation then failed two independent controls. Byte-identity on the real corpus is reported **with** a positive control (`OK U1 67 bytes` → `NOLEAD U1` on a metadata fixture), because an unchanged diff alone is equally consistent with a change that does nothing.
...
**Note on versioning.** There is no 2.46.5 or 2.47.0 entry. Both were built and installed locally but never pushed, tagged or released, so nothing depends on either; this entry is written self-contained and covers the whole window. Reconstructing retrospective entries is left as a deliberate decision rather than done silently — the same call made at v2.45.1.

**2.46.5** carried one body-only change to `/auto`, folded in here rather than given its own entry: the D7 judgment-ledger instruction now **pins a required frontmatter block** (`type`, `date`, `arc`, `status`, `tags`). It previously pinned the ledger's path and no schema at all, so the block was session-authored — 19 of 71 ledgers carried frontmatter, and those 19 used 23 distinct keys agreeing on none beyond four. `status` is the field that makes the corpus queryable: *which arcs still have judgments waiting on a ruling*, previously answerable only by reading prose. `tags` are required because a ledger is a knowledge artifact and belongs in the tag index beside `/prospect` and `/retrospect` logs. The tag shape — family, then artifact-kind, then project, then topics — was derived from a census rather than from one file: `auto` leads 13 of 16 tagged ledgers, while `judgment-ledger` and the older `judgments` split 8/7. An earlier draft pinned a different set and called it "the convention the corpus already uses"; that was measured false, and the caveat against deriving a convention from a single ledger now ships in the instruction itself.
...
The gate's contract is domain coverage: deny the first fetch per session aimed at a domain the knowledge folder or memory dirs already cover. `bin/pre-external-fetch-check.sh` had two branches feeding one pattern. The URL branch extracted a registrable domain and grepped for it literally — sound. The query branch took the four longest non-stopword query words and built `(w1|w2|w3|w4)\.[a-z]` — **the URL branch's domain shape, applied to prose**. Against prose that reads "any query word followed by a dot and a lowercase letter", which matches every `guidelines.md`, `screening.md` and dotted path in a markdown corpus.

**Measured cost:** three consecutive medical-literature searches denied in one session, citing mobile-notification prospect logs and design-token decision docs — zero topical relation. The entire match was the substring `guidelines.m`, seven times: the word "guidelines" inside a *filename*. Isolated per word, `guidelines` hit 4 files while `screening` and `participation` hit 0 — one common English word did 100% of the work. Two of the three lost citations were never recovered.

**Word filtering cannot fix this, and that was measured rather than assumed.** The design intended dictionary-filtered vendor stems, but `/usr/share/dict/words` holds `guideline` and not `guidelines`, so every English plural reads as a vendor stem — while `render` *is* in the dictionary, so filtering would also stop detecting `render.com`, a vendor a real corpus covers. Wrong in both directions. Also rejected: requiring all N words to co-occur in one file, still a weak topical proxy over the same category error at higher complexity.

**The fix.** A query gates only on a domain it actually names (`cdc.gov vaccination schedule`, `site:auanet.org …`), where the URL branch's logic applies unchanged; otherwise the gate stays silent. The TLD allowlist is load-bearing — a generic `\.[a-z]{2,}` reintroduces the bug by matching `screening.the`. Requiring a real TLD is what makes a match *mean* "domain".

**This also repaired the documented retry promise, which was structurally false.** The cooldown key was the four longest words, so *rephrasing* a denied query minted a different key and denied again — punishing the natural response to a blocked search, and the reason one denial became three. Keying on the domain means a rephrased query about the same domain reuses the cooldown, so "the retry will pass" is now true. The reason text's `(${EF_KEY})` also stops naming a word-blob that is not a file.

**A second defect, in the tests rather than the code.** Two assertions encoded the buggy behavior as an acceptance criterion (a bare vendor *word* denying), and after the fix a third would have passed **vacuously** — with prose never gated, its "shares the cooldown" check returns empty whether or not a cooldown exists. Rewritten to the narrowed contract, plus three genuinely new assertions: prose-without-a-domain never denies, a file extension does not satisfy the TLD list, and a prose query and a URL on one domain share one key. A new fixture file holds `guidelines.md` / `screening.md` so the negative fails for the right reason — without it the assertion is green whether or not the fix exists, the same trap the existing stopword traps were added for.

**Validation.** Parse check clean; suite 139 passed / 0 failed (was 136); the 3 stale assertions observed RED before any test edit, and the 2 new negatives observed RED under a mutation that was restored byte-identical via `cmp`. Live end-to-end on the real corpus: all three original denied queries now allow, a prose query naming a covered domain denies, and the **positive control** (a URL on a covered domain) still denies with real paths — without that control, a fix that disabled the gate entirely would look identical to one that works. Note the handoff's suggested `cdc.gov` control was checked first and would have been misleading: the corpus has zero `cdc.gov` mentions, so "allow" is the correct answer there, not a failure.

**Transferable lesson.** A pattern borrowed from a branch where it is correct can be silently wrong in the branch it moves to; `\.[a-z]` is exactly right for a registrable domain and meaningless for a prose word, and because both branches fed one pattern consumed by one grep, nothing marked the boundary where the meaning changed. The diagnostic tell needed no code reading: the "surface" name *changed when one word of the query changed*. A genuine reference to a stored file cannot depend on how the question was phrased — if a gate's identifier tracks the input rather than the thing it claims to have found, the identifier is synthesized. Same class as a guard scoped to the wrong unit: the threshold was never the problem.

Also in 2.45.1 — **`/interview` asks adaptively by default.** The skill offered two cadences and *recommended* the all-at-once one in three of four cases, so the effective default was a numbered wall of questions answered by scrolling and referencing numbers — and because a batch is derived before any answer arrives, it kept asking things an earlier answer had already made irrelevant. The default is now `guided`: small dialogs of 1–4 questions via the platform's question/picker affordance, each question carrying suggested answers plus the always-available custom fill, re-derived after every dialog. Cluster size is gated on mutual independence — anything that branches goes alone. `--socratic` pins the grain to exactly one question per dialog for when the clustering judgment shouldn't be trusted; `--battery` keeps the old all-at-once set as an opt-in, mentioned once and only past ~12 questions. Two cadences, three spellings: they differ on *when* questions are derived, not how many appear at once.
...
**`/audit-knowledge` — a capture-derived finding must be re-verified against HEAD before it is presented as live.** A transcript capture records a *moment*, not current state, and the session it captures very often fixed the thing minutes later. Steps 2d and 2e now require checking the current source before any present-tense defect claim reaches review, marking each **STILL LIVE (verified at HEAD)** or **ALREADY FIXED (closed by `<sha>`)**. Basis: two capture-derived findings were surfaced as live security hazards when a commit that same day had already closed both by name — under a standing grant to file findings without asking, that would have filed two duplicate security tickets. A grant makes the check more necessary, not less. Adversarial `/prospect` and `/retrospect` captures are the densest source of this, since finding defects is their job.
...
- **Three orthogonal axes, one word each — plus `tickets`.** **Authority** `full` (maximum latitude on every axis **except push**, which D4 makes ungrantable by any modifier; it raises the three Step 5 fan-out stopgaps but does not remove them, since an unattended max-authority run is the case most exposed to unbounded spend). **Presence** `attended` | `unattended` — is a human reachable? Two-valued, **always stated in the arc contract**, asked by `config` when unspecified, defaulting to `attended`. It decides what happens to every question the run produces: `attended` surfaces a non-blocking residual immediately, `unattended` batches it to the handoff and arms a resume. This narrows D6 rather than repealing it — the run still never idles waiting for a reply. **Duration** `continue` | `stop`. And `tickets` binds work selection to the connected tracker with a ticket comment per commit. Every combination is expressible: `/auto full attended`, `/auto full unattended continue`, and so on.
- **No `loop` modifier — deliberately.** A draft had one meaning "unattended + continue + self-restart", but once resume-arming moved to the presence axis where it belongs, `loop` reduced to a strict alias for `unattended continue` — a word carrying no capability. It had already produced the drift a redundant word invites (both `full` and `loop` claimed to arm the resume), and it was the worst of the mid-prose collisions. Retiring it also deleted three contradiction rules (`loop stop`, `loop attended`, "loop sets three axes"). Tombstoned in the skill with its rationale, and guarded by an assertion so it is not reintroduced from the modifier list alone.
- **Modifiers are recognised only at the ENDS of the invocation, never mid-prose.** They were previously matched anywhere in the args — so `/auto fix the render loop bug` silently became an unattended self-restarting run, and "do a full review" or "close the tickets" the same way. Now only the contiguous run at the start (after any mode keyword) and the run at the end are scanned; once goal prose begins, every remaining token is goal.
- **A bare `/auto` opens the `config` picker instead of inferring a goal.** With no mode keyword and no goal text, guessing one from `SESSION.md` is exactly the stale-resume hazard "verify before you trust" warns about — a saved prompt may describe work already shipped. Modifiers and toggles do not count as goal context (they say *how*, never *what*), so `/auto`, `/auto full` and `/auto unattended` all open the picker, **pre-seeded** with whatever was typed.
- **Rule 22 bypasses are recorded and reported.** The in-place-write guard stays warn-only — denying would block legitimate work — but a warning that is ignored previously left no trace, and an ignored gate is the failure the guard exists to catch. Each detection now appends one line to `${TMPDIR}/aria-r22-bypass-<session_id>` (no analysis, no blocking, best-effort), and `/handoff` and `/wrapup` report the count and idioms at close.
- **Pending handoffs are checked at three points** — `/handoff`, `/wrapup`, and resume. `/wrapup` matters most: a clean close does not consume another session's pickup, so it is the last chance to notice an unconsumed prompt before the session is gone.
- **The tautology guard covers shell self-comparisons** (`[ $a = $a ]`), restored after being dropped by the portability rewrite that removed regex backreferences. This repo's own guards are shell, so a self-comparison inside a repro suite is precisely the false green the hook exists to catch. Its header also now documents a **known limit**: field extraction stops at the first escaped quote, so a file containing a double quote is only partially examined — failing toward a missed warning, never a spurious one, and stated rather than hidden.
...
- **Two more free-text extractions were escape-unsafe, and the guard was blind to them.** `task_subject` and `task_description` in `task-context-check.sh` are genuinely free text and were silently truncating. The root cause is worse than the two sites: the extraction guard matched a field list **written from memory** rather than derived from the code, so it was structurally incapable of detecting members of the very class it names, and reported green while they were broken. Guard widened to all six free-text fields, and its header now records the derivation command so the list is enumerated rather than recalled. Remaining `echo` sites re-verified by enumeration — identifiers, numerics and POSIX paths, none of which can carry an escape.
- **`/prospect` and `/retrospect` are tracker-agnostic too.** The same vendor lock the `/auto` fix removed also ran through both review skills — 20 and 10 sites respectively, including the `linear` scope, the `--linear-post` flag, "Linear MCP", "Linear-style ticket IDs", "Linear comment", and a hard-coded `mcp__linear__list_comments` call. Canonical names are now `ticket` / `--ticket-post`; **every legacy spelling (`linear`, `--linear`, `--linear-post`) remains an accepted alias**, since those are documented CLI contract and dropping them silently would be its own defect. Vendor names survive only inside sibling lists, and the tracker MCP is probed rather than assumed. New repro `tracker-agnostic.sh` (13 assertions) enforces lock-vs-mention across all three skills *and* asserts the aliases still work.
...
- **SESSION.md holds several valid next-session prompts again — and `/handoff` may no longer skip writing one.** Three compounding gaps meant a handoff was routinely abandoned rather than preserved: nothing anywhere forbade *skipping* SESSION.md to avoid clobbering (the worst outcome — your opener is lost entirely); the demote trigger only fired on `lastEvent: handoff` while the live on-disk state is almost always `in-progress`; and the writer was told `sessionId:` was optional even though the demote guard keys on it, making that guard unevaluable and so read as "does not apply". Now: **a never-skip invariant** in both `/handoff` and `/wrapup`, an absent `sessionId` treated as *someone else's*, `sessionId` **required**, and an explicit rule to **never demote a `lastEvent: in-progress` marker** (it is a live breadcrumb, not a handoff).
  **Unconsumed prompts are kept at full fidelity** — the old one-line collapse degraded a mandate nobody had used yet. That required fixing a **confirmed leak** in `kt_ss_ledger_prune`: a consumed block containing a column-0 `##` lost its boundary and leaked its tail into the file. Block boundaries are now **declared** by an explicit `<!-- aria:entry-end -->` terminator rather than inferred, so a stored prompt may safely contain headings and nested fences; a first pass detects legacy files and keeps the original rule for them, so nothing is orphaned. The section is renamed `## Prior sessions` → **`## Pending handoffs`** — these are still-valid prompts awaiting use, not history, and "prior" is what made demotion read as a downgrade worth avoiding. `session-start-check.sh` now also surfaces unconsumed pending handoffs at resume; a prompt stored but never offered is lost in practice. `session-state.sh` 33 → 44 assertions.

### Notes

- Gate B skill-discovery budget **18,938 → 18,814 bytes** — *down*, despite this release adding a mode, three axes and a tracker-resolution mechanism. The `/auto` description was rewritten from 1,248 B to 1,143 B, paying for the new vocabulary and returning 124 B; the `/prospect` declass returned another 19 B. Headroom 6 B → **130 B**. Every figure measured before writing, not estimated.
...
- **Hook input extraction is now escape-safe.** All four hooks that read a free-text field (`command`) use `printf '%s'` rather than `echo`. The hazard was measured, and the measurement **corrected an earlier overstatement in this arc's commit messages**: `bash`'s `echo` leaves backslashes alone while POSIX `sh`'s interprets them, and `plugin.json` invokes every hook as `bash …` — so the affected hooks were **not** broken in production, contrary to what commit `dc37b91` claimed. The real defect is that every `bin/` script declares `#!/bin/sh` while only ever being exercised under `bash`: the shebang and the invocation disagree, so the scripts are correct by accident of how they are called, and anything running one directly or under a POSIX `sh` gets a silent fail-open. Fixed at the two free-text sites (`bash-cd-check.sh`, `post-push-retrospect-check.sh`); the ~18 sites reading `session_id` / `file_path` / `transcript_path` are deliberately untouched, since those values cannot contain an escape and `echo` and `printf` are equivalent there. New repro `hook-json-extraction.sh` proves the divergence on the host, proves `printf` is safe under both shells, and pins the rule.
...
- **Rule 35 — the Step 4/5 bar binds BOTH branches.** Routing a fork to the human had become an escape hatch from the analysis: an *incomplete* option set forces the human to redo your work to find the option you missed, and an *unfiltered* one offers a known-bad option as a co-equal choice. Both spend the turn the routing table exists to protect. New **filter clause** — an option carrying an objective, validated, provable defect is a finding, not a choice; present only what survives the Step 3 criteria and record what was filtered in one line *beside* the question, never as a row inside it (silent filtering costs the human the ability to catch a bad filter, which is much of why you asked). Two carve-outs (only-viable-option; clear overpowering offset) and a falsifiability bar: *if you cannot state the defect in one falsifiable line, it is a preference, not a proof — the option stays.* New **malformed-ask diagnostic** — pushback that yields a dominating option which you then produce immediately, with no new information gathered, proves the option was derivable before you asked; audit the routing rather than proceeding gratefully. The usual cause is a false dichotomy whose properties were never actually coupled.
- **Rule 35 — the `autonomy` levels are specified in the rule for the first time.** `default` / `balanced` / `autonomous` semantics previously existed *only* inside the SessionStart shell strings, with no human-readable spec anywhere. Now a 3-row table — and load-bearing, since Rules 18 and 38 both defer to it by name.
- **Rule 38 "Close the class, not the instance" — promoted to canonical.** A fix that provably leaves potential for the same class of bug elsewhere is not viable; viability requires closing the *class*, and elimination-of-potential sits above "does it work." Carries the four-vector recurrence test (sibling call-site / disagreeing source of truth / stored-data residual / unversioned artifact) with its matching closures (census, unify, backfill-or-grandfather, version), the "this is not scope creep — it is the acceptance bar" framing, the product-fork carve-out, and five worked examples remapped onto the four vectors so they read as translatable rather than as a web-only checklist.
- **Rules 18 + 38 — the escalation route an absolute acceptance bar was missing.** Rule 38 as promoted left an agent facing a much larger foundational fix with only two rule-breaking moves: patch silently, or unilaterally absorb the scope. Rule 18 now routes a *contested* foundational-vs-patch call to the human at `default`/`balanced`, naming both failure modes (quietly patching **and** quietly absorbing a much larger scope). Rule 38 gains the matching waiver: **the bar is the agent’s to meet, not the agent’s to waive** — only the human waives it, explicitly. Under `autonomy: autonomous` there is no waiver to seek: take the foundational fix and absorb the scope, escalating only when doing so would change **what the arc IS** (scope boundary, deliverable, completion criteria) rather than merely make it bigger.
- **`user-rules.md` is now always in force.** It previously reached context on exactly two paths — inside `/auto` (Step 0.4) and on a `/rules` lookup — so in an ordinary session a user’s standing rules were simply absent, and the only instructions for fixing that lived *inside the unloaded file itself*. The SessionStart hook now injects the user’s U-rule **headers** as an always-loaded index, with bodies read on demand (two-tier, matching the memory-index pattern; a mature `user-rules.md` runs 50 KB+). Absent file or zero rules injects nothing, so a fresh install pays nothing. Applied to the `plugin-claude-code`, `plugin-openai-codex`, and `plugin-cursor-template` hooks.
- **`user-rules.md` template now ships empty.** The four sample rules (`U1` linter / `U2` fixtures / `U3` return types / `U4` `[RETIRED]` yarn) read as real directives on a fresh install. Removed, leaving the preamble plus an empty landing section. Six consumer sites across five `SKILL.md` files converted from "matches the shipped sample text" to a simpler, more robust empty-state test.
- **SessionStart injection** also gains the filter/presentation clause on *both* postures plus the foundational carve-in on `autonomous`.

Gate A: 32 suites + 70 tests, 0 failed. Gate B unaffected — body-only `SKILL.md` edits, no frontmatter description changes (surface stays 18,938 B against the 18,944 B budget). Minor bump (new canonical rule + new always-on injection = new user-inheritable capability surface, per the v2.41.0 precedent). **Ports:** Claude-Code-canonical release. Rule 35 and Rule 18 were applied to all five `template/rules/working-rules.md` copies in source and the injection to all three port hooks, so codex/cursor/antigravity/cowork carry the changes in-tree and ship them on their own cadence. Rule 38 was deliberately **withheld** from the three 35-rule ports — antigravity/cursor/codex still lack canonical Rules 36–37 from v2.38.0, so adding 38 there would open a numbering gap; it ships in `plugin-claude-code` and `plugin-claude-cowork` only.

## 2.41.0 — 2026-07-19

**New `/audit usage` sub-audit + `bin/usage-metrics.sh` — a value/ROI report over your OWN knowledge corpus.** The published `docs/value-analysis.md` is the plugin author's N=1 evidence digest (cost-surface token math + prospect/retrospect quality distributions + confound-aware analysis); until now a plugin user had no way to generate the equivalent for their own corpus. `/audit usage` closes that gap.

- **`bin/usage-metrics.sh`** — deterministic emitter (POSIX sh). Reads `KT_KNOWLEDGE_FOLDER` via `config.sh` and prints a labeled metrics block: prospect verdict distribution (+ month buckets), retrospect outcome distribution + per-fix-verdict rate (+ month buckets), canonical pattern count, per-content-dir corpus counts, skill-discovery byte surface, and audit-pass count. The single source of truth for the value-analysis reproduction recipe; grep keys verified against the live corpus. Zero-corpus / missing-folder paths emit zeros / a single error line, never crash.
- **`/audit usage`** (new sub-audit under the existing `/audit` dispatcher) — shells out to the metrics script once, then writes a **sample-size-honest** analysis to `{knowledge_folder}/references/usage-analysis.md` (stamped with `synthesized_at` + `measured_at_corpus`). Trend tables render only for months with ≥10 logs; a zero corpus stops with a "run some cycles first" message rather than fabricating. Renders an inline summary. **Opt-in only** — never fired by a cadence nudge, exactly like `/audit style`.
- **Dispatcher** — `/audit usage` added to the grammar, bare-menu (option 4), delegation, `/audit all` (now four legs + tally clause), unknown-verb list, and the never-auto-run rule.
- Carries the standard ADR-094 Runtime Gate (Cowork-native variant: `/aria-cowork:audit-usage`).

New `tests/test-usage-metrics.sh` (15 assertions: distributions, corpus, cost, zero-corpus, missing-folder); full suite 70/0. Skill-discovery surface 18,938 B — within the 18,944 B Gate B budget (no raise needed; ~6 B headroom, so the next skill addition will require a trim or a deliberate raise). Minor bump (new user-inheritable capability surface, per the v2.40.0 precedent). **Ports:** Claude-Code-canonical only; codex/cursor/antigravity/cowork tracked-drift (reach it at the next parity pass).

## 2.40.2 — 2026-07-17

**Bugfix — `/prospect`, `/retrospect`, `/foundational-review`, `/readiness-audit` now resolve every log path from the configured `knowledge_folder`, never a phantom `~/knowledge/`.** Two long-standing defects in the log-writing / pattern-library-reading instructions of these four skills: (1) a **phantom path literal** — the write target and related-detection glob resolved to a bare `~/knowledge/logs/…` tree *outside* the user's versioned knowledge repo, so prospect/retrospect logs silently accumulated unversioned (40 such logs were discovered and rescued into the real repo on 2026-07-17); (2) a **nonexistent config field** — the resolve instruction cited `knowledge_root`, a field that does not exist in `aria-knowledge.local.md`; the real field is `knowledge_folder`. The Step-0.5 surfacing code already resolved `knowledge_folder` correctly (`<knowledge_folder>/index.md`) — only the write/read instructions were wrong. **Fix:** every `~/knowledge/` path literal → `<knowledge_folder>/` and every `knowledge_root` field reference → `knowledge_folder`, mirroring the already-working Step-0.5 idiom verbatim — so a `/prospect` run now resolves its log to `<knowledge_folder>/logs/prospect/…` (e.g. `~/Projects/knowledge/logs/prospect/…`). Lazy-create + grandfathering behavior preserved; per-port config-file *locations* (`~/.claude/aria-knowledge.local.md`, `~/.gemini/antigravity/…`, `<knowledge_folder>/aria-config.md`) preserved — only the field *name* and path semantics corrected. **Applied identically across all 4 ports** (plugin-claude-code + plugin-antigravity + plugin-openai-codex + plugin-claude-cowork) since the bug was byte-faithful in every port per ADR-013; 20 files, 93/93 pure token swaps, zero structural change. The scope-widening from "prospect/retrospect Step 6" to also `/foundational-review` + `/readiness-audit` was caught by a whole-plugin residual sweep — those siblings carried the same dead-field bug in `<knowledge_root>` placeholder form (incl. an *independent* log-write at `foundational-review-chain.md`). **Release ceremony:** Claude-Code-canonical (bump + tag + GH release). Verified: zero `~/knowledge/` and zero `knowledge_root` remain anywhere in `plugin-*/skills/`; dry-check resolution lands in the versioned repo. Patch bump 2.40.1 → 2.40.2 (body-only bugfix — no frontmatter/description change, Gate B skill budget unaffected).

## 2.40.1 — 2026-07-16

**`/auto` now loads and applies the user's standing `user-rules.md` for the whole arc.** `/auto` was deeply wired to **Rule 35** (the plugin-managed decide-vs-ask *routing* policy) but had zero awareness of `user-rules.md` (the user's own *substantive* standing rules) — so an autonomous arc could run without consciously applying rules the user had already written down. New **Step 0.4 (Load the standing user rules)** resolves `knowledge_folder` from `~/.claude/aria-knowledge.local.md` (same resolution `/rules` uses), reads `{knowledge_folder}/rules/user-rules.md` if present, and holds each real rule as a **binding constraint applied wherever contextually relevant** across the arc — where a user rule speaks to the situation, it governs. **One carve-out:** a user rule does not govern where applying it would be actively detrimental or would contradict validated empirical proof whose violation carries critical impact — and even then the resolution is a **legitimate stop** (surface the rule + the opposing proof + the impact and let the user decide), never a silent override. A missing/sample-only file is treated as "no standing rules" and is never a stop. **Compose-not-duplicate:** `/auto` loads and applies; it never restates the rules' content or re-implements `/rules`. The arc contract (Step 0.5) gains a **"Standing rules loaded"** line so the applied rules are legible, and a Notes bullet distinguishes the two rule sources (Rule 35 = routing; user-rules.md = substance).

**`/audit style` card wording fix — "laws" → "Your Rules".** The shareable "Your Working Style" card (Step 5, Part B) used "law(s)" as its term for a passed rule, producing headings like "Work Laws" — inconsistent with the "rules" vocabulary used everywhere else in the skill (the receipts gate, the rules backlog, the disposition prompt) and with the sibling `/rules` skill's "Your Rules" framing. Renamed the three card headings to **Your Rules — Work / Design Taste / Writing Voice** and changed every remaining prose "law(s)" reference (Reasoning Type, Through-line, Corroboration, Blind spots, Evolution, Consistency, Anti-patterns) to "rule(s)" so the card's vocabulary is internally coherent. The anti-ditto-copy derivation procedure on element #1 (added in the v2.40.0 dogfood refinements) is preserved untouched.

**Both body-only edits — Gate B skill-discovery budget unaffected** (frontmatter descriptions unchanged). Patch bump 2.40.0 → 2.40.1 (wording + body-only, per the v2.39.1 precedent). **Ports:** Claude-Code-canonical only (`/auto` and `/audit style` both ship only in plugin-claude-code; codex/cursor/antigravity/cowork tracked-drift).
