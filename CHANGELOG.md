...
`setup` SKILL extended at five surfaces: a new **Step 6b** that fires once on fresh install or first post-upgrade run with a focused y/n prompt (so the new feature isn't dormant by default), Step 6 advanced-options listing for re-run discoverability, Step 7 frontmatter write, Step 7 formatting rules, and Step 7b round-trip + empty-sentinel verification. Step 6b detects missing-key cases via `grep -q '^ticketing_plugins:' ~/.claude/aria-knowledge.local.md` and skips silently if the key is already present (empty counts as a deliberate skip — returning users aren't re-prompted). Step 8 summary line confirms the disposition. Plugin tags follow the same `:`/`,` exclusion as `projects_list`; plugin-command values must be bare command names without leading `/` (the audit prepends the slash when printing the hint).
...
- **`ticketing_plugins` prompted on first post-upgrade `/setup`.** The new Step 6b detects missing-key configs (upgrades from <2.12.0) and runs a focused y/n prompt — answering `n` writes empty and the prompt never returns; answering `y` collects the mapping inline. Existing configs without the key get the prompt exactly once. Plugin-command values are bare names (e.g., `foo-ticket`, not `/foo-ticket`). To change the value later, re-run `/setup` and request advanced options.
