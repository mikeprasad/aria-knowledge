#!/bin/sh
# pre-external-fetch-check.sh — preToolUse hook (Cursor port of Claude Code's
# PreToolUse:WebFetch|WebSearch local-reference interrupt).
#
...
# passes. Cursor preToolUse CAN deny (`permission: deny`); that is the native
# equivalent of Claude's permissionDecision deny.
#
# Cursor-native notes:
#   - Payload uses camelCase sessionId; snake_case is a fallback.
#   - URL/query may appear as url, query, or search_term (Cursor WebSearch).
#   - Corpus: knowledge folder + ~/.claude/projects/*/memory (if present) +
#     ~/.cursor/projects/*/memory (if present). Agent transcripts are NOT
#     scanned — they are too large and are not the "recorded reference" unit.
#   - Gate ships OFF (external_fetch_gate: off).
#
...
[ -z "$QUERY" ] && QUERY=$(printf '%s' "$INPUT" | grep -o '"search_term":"[^"]*"' | head -1 | sed 's/.*"search_term":"//;s/"$//')
...
EF_CURSOR_MEMDIR="${ARIA_EF_CURSOR_MEMDIR:-$HOME/.cursor/projects}"
...
EF_HITS=$(
  {
    ...
    for d in "$EF_CURSOR_MEMDIR"/*/memory; do
      [ -d "$d" ] || continue
      /usr/bin/grep -rlE "$EF_PATTERN" "$d" 2>/dev/null
    done
  } | LC_ALL=C sort -u
