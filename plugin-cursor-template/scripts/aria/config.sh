...
KT_ROOT="${KT_ROOT:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
KT_CONFIG="${KT_CONFIG:-$KT_ROOT/.cursor/aria-knowledge.local.md}"
KT_CONFIGURED=false
KT_CONFIG_ERROR=""
...
if [ -f "$KT_CONFIG" ]; then
  ...
  # Preflight commit gate. Default WARN: escalation to deny is opt-in.
  # Unrecognized values fall back to warn rather than to off — a typo must not
  # silently disable a gate.
  case "$KT_PREFLIGHT_GATE" in
    off|warn|deny) : ;;
    *) KT_PREFLIGHT_GATE="warn" ;;
  esac
  case "$KT_EXTERNAL_FETCH_GATE" in
    on|off) : ;;
    *) KT_EXTERNAL_FETCH_GATE="off" ;;
  esac
  case "$KT_EXTERNAL_FETCH_MAX_HITS" in
    ''|*[!0-9]*) KT_EXTERNAL_FETCH_MAX_HITS=8 ;;
  esac
  ...
  #   (the hooks still apply their built-in planning globs, e.g. docs/specs, .cursor/skills/*/templates)
  # KT_PREFLIGHT_DENY_PATHS / KT_PREFLIGHT_DENY_REPOS have no default — empty means no escalation
  ...
  case "$KT_TASK_BOUNDARY_CAPTURE" in
    true|false) ;;
    *) KT_TASK_BOUNDARY_CAPTURE=true ;;
  esac
  case "$KT_EDIT_INTENT" in
    true|false) ;;
    *) KT_EDIT_INTENT=true ;;
  esac

  # Derived paths for task-boundary captures and the edit-intent marker.
  # Computed AFTER knowledge_folder validation so they only resolve when usable.
  if [ -n "$KT_KNOWLEDGE_FOLDER" ]; then
    KT_TASK_CAPTURE_DIR="${KT_TASK_CAPTURE_DIR:-$KT_KNOWLEDGE_FOLDER/intake/task-boundary-captures}"
  fi
  KT_EDIT_INTENT_FILE="${KT_EDIT_INTENT_FILE:-$KT_ROOT/.cursor/aria-edit-intent.json}"
fi
...
# kt_project_menu
# Build the SessionStart picker menu display string from KT_PROJECTS_LIST (+ optional
# KT_PROJECTS_LABELS). Each entry renders "tag (Label)" when a label exists for the tag,
# else bare "tag". Comma-space joined. Empty list -> empty string. Pure function.
kt_project_menu() {
  [ -z "$KT_PROJECTS_LIST" ] && return
  _kt_menu=""
  _kt_old_ifs="$IFS"
...
# declare an active batch by writing .cursor/active-batch.json. The pre-edit
...
KT_BATCH_MANIFEST="${KT_BATCH_MANIFEST:-$KT_ROOT/.cursor/active-batch.json}"
...
kt_batch_begin() {
  ...
  mkdir -p "$KT_ROOT/.cursor" 2>/dev/null
