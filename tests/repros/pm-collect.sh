...
mkdir -p "$PM_TMP/root"; make_fixture "$PM_TMP/root/relproj" "$(utc_iso $((NOW-86400)))"  # 1d ago, RELATIVE entry
...
# The 'rel' entry is a RELATIVE path resolved against pm_projects_root (mirrors the real config shape).
export KT_CONFIG="$PM_TMP/aria-knowledge.local.md"
cat > "$KT_CONFIG" <<EOF
---
knowledge_folder: $PM_TMP/knowledge
projects_enabled: true
projects_list: fresh:$PM_TMP/fresh,stale:$PM_TMP/stale,closed:$PM_TMP/closed,rel:relproj
pm_projects_root: $PM_TMP/root
...
assert_eq "relative entry -> ACTIVE"   "ACTIVE"  "$(jq -r '.projects[]|select(.name=="rel").tier' "$OUT")"
assert_eq "relative path resolved"     "$PM_TMP/root/relproj" "$(jq -r '.projects[]|select(.name=="rel").path' "$OUT")"
pm_summary
exit "$PM_FAIL"
