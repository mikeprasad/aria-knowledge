---
name: prospect
description: "Run a structured pre-mortem on a plan, task, branch, file, ticket, or session before execution. Trigger on /prospect, pre-mortem, risk review, or check this plan."
...
When the skill is *not* directly invoked, Codex monitors user messages and the conversation state for cues that suggest a pre-mortem is warranted. When detected AND the current session has produced a multi-step plan with no execution yet, Codex offers — never auto-executes — `/prospect`.
