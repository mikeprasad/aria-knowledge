---
name: auto
description: "Drive an autonomous execution arc end-to-end — compose brainstorm→spec→/prospect→plan→/prospect→TDD→/retrospect under the Rule 35 posture, decide objectively-validatable forks yourself, and stop only on a load-bearing fork or an ungranted approval. Modes: `arc` (default), `execute <plan|spec|ticket-id>` (skip ideation), `plan` (stop at a prospected plan, no code), `config` (guided per-run knob..."
...
- **D1 — Usage budget is explicit in Codex.** Claude Code's statusline-derived 5-hour/7-day
  figures are not available in the Codex plugin surface. Do not read or infer a hidden usage
  percentage. If a run needs a hard budget, ask the user for a time/task/turn cap up front and
  report progress against that cap. If no cap is provided, checkpoint frequently and use
  `/handoff` before context pressure makes continuity fragile.
...
- **`attended` / `unattended`** — the **presence** axis: is a human reachable right now?
  ...
  **Two different walls need two different mechanisms — do not conflate them:**
  ...
  |---|---|---|
  | **Explicit cap** — the user-provided cap is reached | the **resume schedule** if available (Step 6) | Re-fires *this* session after the scheduled time, local work intact |
  | **Context** — continuity is getting fragile | `/handoff` | Produces a durable opener for a fresh Codex task |

  A resume schedule cannot rescue a context wall because it re-enters the same full session.
  Codex does not bundle the Claude self-restart wrapper, so the context-wall answer is a clean
  `/handoff`, not a promised automatic restart.
...
**Context-self-restart flag** (a trailing `self-restart` keyword): accepted for cross-port
compatibility, but **non-equivalent in Codex**. It does not write a restart signal or launch a fresh
process. On a context wall, run `/handoff` and stop cleanly.
...
## Step 3: Budget — Codex explicit-cap mode

Codex does not expose Claude Code's statusline usage snapshot to plugins. The native equivalent is
explicit-cap discipline:

- **Context pressure** → AUTO-run `/extract` when useful, then `/handoff` to a durable on-disk opener
  before continuity gets fragile.
- **User-provided cap** → if the invocation names a time/task/turn cap, report progress against that
  cap between work units and stop at a clean checkpoint when it is reached.
- **No provided cap** → do not invent a percentage. Keep batches small, checkpoint durable state, and
  ask only when a real stop condition from the standing directives appears.

## Step 3¾ (optional): Context-self-restart across a fresh process

Default OFF. **Claude Code only in the canonical implementation.** In the Codex port, a skill cannot
launch a fresh Codex task or reset its own context, so `self-restart` degrades to the normal Step 3
path: extract if useful, write `/handoff`, and stop cleanly.

The canonical Claude implementation solves unattended context walls with a fresh `claude` process
launched by an external wrapper. Codex has no bundled equivalent in this plugin, so do not write the
Claude restart signal or promise an automatic restart.
...
- **Budget-fraction pre-flight gate becomes explicit-cap gate in Codex.** Before launching ANY fan-out
  (a parallel sweep, many independent subagents, or an opted-in workflow), estimate the burst size
  against the user's explicit cap if one exists. If no cap exists and the fan-out is broad enough that
  it could dominate the run, STOP and surface options: proceed, shrink to a smaller batch, serialize,
  or skip. Do not cite a statusline percentage in Codex.
...
| **launchd** (the `pm-schedule.sh` pattern) | Claude Code only; not bundled in Codex | Yes — OS-level | Yes — a fresh `claude` invocation | Non-equivalent in this Codex port |
| **`bin/auto-runloop.sh`** (`self-restart`) | Claude Code only; not bundled in Codex | Wrapper-dependent | Yes — a fresh `claude -p` process | Non-equivalent in this Codex port |
...
- **Tool approval companion.** For unattended runs, use Codex's normal approval/trust mechanisms for
  plugin hooks and shell commands. This Codex port does not write runtime permission settings.
