---
name: audit-style
description: "Mine provided Codex transcript/log history for evidence-gated working-style rules and stage candidates to the rules backlog. Requires explicit transcript corpus in Codex."
...
**Locate the corpus.** Codex does not document a stable historical transcript corpus equivalent to Claude Code's `~/.claude/projects/<cwd-encoded>/*.jsonl`. Prefer an explicit user-provided transcript/log directory. If none is provided and a local Codex transcript corpus is discoverable, state the path and treat it as best-effort; otherwise stop and explain that `/audit style` needs exported or provided Codex transcripts.
