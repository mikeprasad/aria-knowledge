...
- `last_setup_version` is a semver string read from `${ARIA_PLUGIN_ROOT}/.codex-plugin/plugin.json` at Step 1 — write it as bare digits-and-dots plus any Codex prerelease suffix (e.g., `2.46.2-codex.0`), not quoted, not prefixed with `v`. The session-start hook compares this against the installed plugin version to detect upgrades since the user's last `/setup`
