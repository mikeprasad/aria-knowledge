...
# relying on the agent executing a soft SessionStart instruction (v2.22.0's approach,
...
#                                           AGENTS.md, CLAUDE.md, or PROGRESS.md
#                                           (the project root), or empty if none
#                                           below $HOME / /.
...
kt_ss_find_root() {
  ...
  while [ -n "$_ss_dir" ] && [ "$_ss_dir" != "/" ]; do
    if [ -f "$_ss_dir/AGENTS.md" ] || [ -f "$_ss_dir/CLAUDE.md" ] || [ -f "$_ss_dir/PROGRESS.md" ]; then
      ...
      # ~/Projects) whose AGENTS.md/CLAUDE.md is the master index, not a project.
      # Marking it would write a spurious root SESSION.md. Real projects live
      # inside it.
