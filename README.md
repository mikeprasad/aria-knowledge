...
- `/retrospect` — Structured retrospective on a shipped commit range, single commit, PR, or current session. Enforces per-fix validation (no fix marked "shipped" without named evidence — log event, reproduction-then-fix-verified, production instrumentation, or deployed-state check), surfaces simpler alternatives and maintenance cost per change, runs a failure-mode pattern check against a growing canonical + project-specific library, re-diagnoses when fixes failed, and emits an action verdict (KEEP / REVERT / REDO-MINIMAL / FOLLOWUP-TICKET / HOLD-PENDING-EVIDENCE). Soft-suggested when the user reports a regression. Output saved to `logs/retrospect/`.
