...
MAIN_MSG='CHANGE DECISION CHECK (change-decision-framework.md) — REQUIRED: your NEXT Edit/Write must be preceded (in the same assistant turn, ABOVE the tool call) by the Low/High Impact block. This hook fires with the tool result and cannot block the CURRENT edit; output the block retroactively for this one AND prospectively for the next. The block is NOT optional for: discuss-then-edit cadences, docs-only edits, in-review / rule-by-rule passes, or sessions where "the conversation already covered it" — the block'"'"'s slots (Change / Solutions / Execute) force specific thinking that conversation does not guarantee. If you conclude "the only way to satisfy the hook is retroactively," you are reading only half the instruction — the AND clause above is the actual requirement. See rules/change-decision-framework.md sections "Ordering (required)" and "Rationalizations that do not apply". --- Assess impact: HIGH (behavior, architecture, key logic, many dependents) or LOW (content, simple functions, docs). --- HIGH IMPACT FORMAT: Line 1: High Impact — [what] ([why high]). Then one line each: Change — [what + context]. Intake — [information gathered]. Criteria — [objective basis]. Solutions — [all options ranked, best first]. Rank — [winner + why]. Validate — [does it hold up? contradictions?]. Execute — [precise scope]. FLAG if Validate or Execute fails and newline with Proposed: or Question: for next step. --- LOW IMPACT FORMAT: Line 1: Low Impact — [what] ([why low]). Then: Change — [what + intake + criteria in one line]. Solutions — [options ranked, best first]. Execute — [decision; scope check, secondary impact check, functional impact]. If Execute flags: add FLAG and newline with Proposed: or Question: for clarification needed.'
