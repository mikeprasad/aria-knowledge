# CHANGELOG — Archive (v2.0.0 → v2.9.1)

Pre-v2.10 entries split out from the main [CHANGELOG.md](CHANGELOG.md) on **2026-05-19** to keep the active changelog scannable. The active CHANGELOG.md continues to live at the repo root and starts at v2.10.0; new entries are prepended there.

For the current changelog (v2.10.0 → latest), see [CHANGELOG.md](CHANGELOG.md).

This archive covers ARIA's first ten months of versioned releases: the initial release sequence, the early rule + hook iterations, the project-tier and shared-knowledge tier additions, and the run-up to the v2.10.0 batch-manifest + structural-signal compaction model that defines the modern Rule 22 enforcement shape.

Per Rule 6 (*"Don't delete — archive"*), this file preserves the full original content of the relocated entries. Nothing was rewritten.
...
Major release absorbing design imports from an execution-first fork variant contributed by a collaborator. ARIA's knowledge lifecycle stays intact; additions are `/distill` spec shaping, `/stitch` cross-repo binding, structural signal surfacing in hooks, and rule-sub-structure extensions. Zero breaking changes; all new features are opt-in or additive.
...
- `/context` skill — on-demand knowledge retrieval by topic tags with OR (default) and AND modes, project tag expansion (e.g., `/context web` expands to all web-app-relevant tags), summary-first presentation with selective file loading
